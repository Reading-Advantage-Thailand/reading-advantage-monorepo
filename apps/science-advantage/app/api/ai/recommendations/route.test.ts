 ...
 *
 * Target: `app/api/ai/recommendations/route.ts` (POST handler).
 *
 * Pinned shape from `measure/tracks/observability_stack_20260603/spec.md`
 * FR-6 (lines 153-166) and `plan.md` Phase 5:
 *   - The top-level handler is wrapped in `runWithRequestContext` so the
 *     `logger.error('ai.recommendation.error', { traceId })` call from
 *     within the catch block carries the request context
 *     (`requestId` / `route` / `method` / `latencyMs`).
 ...
 *     HEAD, so the `logger.error('ai.recommendation.error', { traceId })`
 *     call from the catch block fires outside any `runWithRequestContext`
 *     scope; the captured log line will NOT carry `requestId` / `route`
 *     / `method` / `latencyMs` — the wrap contract assertion fails.
 *
...
vi.mock('@reading-advantage/domain/ai', () => ({
  getRecommendation: vi.fn(async () => {
    throw new Error('phase5-forced-rec-failure');
  }),
}));
...
describe('FR-6 recommendations route (POST) — wrap + structured-logger catch block', () => {
...
describe('FR-6 recommendations route — observability modules are wired (sanity)', () => {
