 ...
 *
 * Target: `app/api/classes/[classId]/lessons/[lessonId]/analytics/route.ts`
 *
 ...
 *     the request context.
 *   - The catch block's `console.error('Error fetching lesson analytics:', error);`
 *     (current source line 29) is replaced with
 *     `logger.error(event, { error, ...otherContext })` so the line goes
 ...
 *     HEAD, so even if the logger fires the captured line will not
 *     carry `requestId` / `route` / `method` / `latencyMs` — the wrap
 *     contract assertion fails.
 *   - The catch block still calls raw `console.error` at HEAD (line 29)
 *     — the "replaced with `logger.error`" assertion fails.
 *
...
vi.mock('@/lib/validations/api-helpers', () => ({
  parsePath: vi.fn((params: Record<string, string>) => ({
...
vi.mock('@/lib/validations/params', () => ({
  classIdLessonIdParamSchema: { _mock: true },
}));

vi.mock('@/lib/auth/server', () => ({
  requireAuth: vi.fn(async () => ({
    user: {
      id: 'phase5-analytics-user',
...
vi.mock('@reading-advantage/domain/classes', () => ({
  getClassLessonAnalytics: vi.fn(async () => {
    throw new Error('phase5-forced-domain-failure');
  }),
}));

interface RouteModule {
  GET: (request: NextRequest, ctx: { params: Promise<{ classId: string; lessonId: string }> }) => Promise<Response>;
}
...
describe('FR-6 analytics route — wrap + structured-logger catch block', () => {
  ...
  it('emits the catch-block error log as a JSON line carrying `requestId` / `route` / `method` / `latencyMs`', async () => {
    const req = new NextRequest('http://localhost/api/classes/c-1/lessons/l-1/analytics', { method: 'GET' });
    const res = await route.GET(req, { params: Promise.resolve({ classId: 'c-1', lessonId: 'l-1' }) });

    const allParsed = findJsonLogStrings(errorSpy.mock.calls);
  ...
  it('does NOT call raw `console.error` from the catch block (replaced with logger.error per FR-6)', async () => {
    const req = new NextRequest('http://localhost/api/classes/c-1/lessons/l-1/analytics', { method: 'GET' });
    await route.GET(req, { params: Promise.resolve({ classId: 'c-1', lessonId: 'l-1' }) });

    const allArgs = errorSpy.mock.calls.flat();
    const legacyPrefixArgs = allArgs.filter(
      (a) => typeof a === 'string' && a === 'Error fetching lesson analytics:',
    );
    expect(legacyPrefixArgs, 'legacy `console.error("Error fetching lesson analytics:", error)` must be removed').toHaveLength(0);
  });
...
describe('FR-6 analytics route — observability modules are wired (sanity)', () => {
