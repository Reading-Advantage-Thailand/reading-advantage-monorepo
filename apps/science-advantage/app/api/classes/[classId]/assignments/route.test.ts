 ...
 *
 * Target: `app/api/classes/[classId]/assignments/route.ts` (GET handler).
 *
...
vi.mock('@/lib/validations/api-helpers', () => ({
  parseBody: vi.fn(async () => ({ lessonId: 'l-1', dueAt: null })),
  parsePath: vi.fn((params: Record<string, string>) => ({ classId: params.classId })),
...
vi.mock('@/lib/validations/assignments', () => ({
  createAssignmentSchema: { _mock: true },
  deleteAssignmentSchema: { _mock: true },
}));

vi.mock('@reading-advantage/domain/classes', () => ({
  listAssignments: vi.fn(async () => {
    throw new Error('phase5-forced-list-failure');
  }),
  createAssignment: vi.fn(async () => {
    throw new Error('phase5-forced-create-failure');
  }),
  deleteAssignment: vi.fn(async () => {
    throw new Error('phase5-forced-delete-failure');
  }),
}));

interface RouteModule {
  GET: (request: NextRequest, ctx: { params: Promise<{ classId: string }> }) => Promise<Response>;
}
...
describe('FR-6 assignments route (GET) — wrap + structured-logger catch block', () => {
  ...
  it('emits the catch-block error log as a JSON line carrying `requestId` / `route` / `method` / `latencyMs`', async () => {
    const req = new NextRequest('http://localhost/api/classes/c-1/assignments', { method: 'GET' });
    const res = await route.GET(req, { params: Promise.resolve({ classId: 'c-1' }) });
    ...
    expect(withCtx!.userId).toBe('phase5-assignments-user');
    expect(withCtx!.route).toContain('/api/classes/c-1/assignments');
...
describe('FR-6 assignments route — observability modules are wired (sanity)', () => {
