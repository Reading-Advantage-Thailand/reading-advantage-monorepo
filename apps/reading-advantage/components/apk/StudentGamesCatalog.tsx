...
/** Props for the Reading student catalog client surface. */
export interface StudentGamesCatalogProps {
  /** Validated tenant and student identity used for RPG requests. */
 ...
 * @param cartridgeId Public cartridge identifier.
 * @returns A public cover path used by the catalog card.
 */
...
/**
 * Renders the Reading student catalog and its compact reward panel.
 * @param props Optional validated RPG owner identity.
 * @returns The existing game cards with catalog reward controls.
 */
export function StudentGamesCatalog({ ownerKey }: StudentGamesCatalogProps) {
  ...
  const renderCards = (games: typeof cartridgeCatalog, showTooltip: boolean) => (
    <div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
      <TooltipProvider>
        {games.map((game) => (
          <div key={game.id} className="h-full">
            <Card className="group relative h-full overflow-hidden transition-all duration-300 hover:shadow-2xl hover:shadow-primary/20 hover:-translate-y-1 hover:z-30 cursor-pointer border-2 hover:border-primary/50 bg-card/50 backdrop-blur-sm flex flex-col" onClick={() => router.push(catalogApkHref(game.id))}>
              <CardHeader className="relative p-0 overflow-hidden shrink-0">
                <div className="relative w-full h-48 group-hover:h-56 transition-all duration-300 overflow-hidden">
                  <Image src={catalogCover(game.id)} alt={game.title} fill className="object-cover transition-all duration-300 group-hover:scale-105" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" />
                </div>
                {showTooltip ? (
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <div className="absolute bottom-4 right-4 z-10 bg-white/90 dark:bg-gray-900/90 p-2 rounded-lg shadow-lg"><Gamepad2 className="h-6 w-6 text-primary" /></div>
                    </TooltipTrigger>
                    <TooltipContent><p>{t("clickToPlay")}</p></TooltipContent>
                  </Tooltip>
                ) : null}
              </CardHeader>
              <CardContent className="pt-6 pb-4 space-y-4 flex-1 flex flex-col">
                <CardTitle className="mb-2 text-2xl font-bold group-hover:text-primary transition-colors line-clamp-1">{game.title}</CardTitle>
                <CardDescription className="text-base leading-relaxed line-clamp-3">{game.description}</CardDescription>
              </CardContent>
              <CardFooter className="pt-0 pb-6 mt-auto">
                <Button className="w-full font-semibold text-base h-11 shadow-md hover:shadow-lg" onClick={(event) => { event.stopPropagation(); router.push(catalogApkHref(game.id)); }}>
    ...
    <div className="max-w-7xl mx-auto mt-8 px-4 sm:px-6">
      <div className="flex flex-col gap-12">
        ...
        <section>
          ...
          {renderCards(vocabularyGames, true)}
        </section>
        <section>
          <div className="mb-6">
          ...
          {renderCards(sentenceGames, false)}
        </section>
      </div>
    </div>
  );
