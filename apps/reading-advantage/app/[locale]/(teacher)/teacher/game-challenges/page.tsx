...
/**
 * Renders the Reading teacher challenge manager.
...
export default async function TeacherGameChallengesPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <main><Link href={`/${locale}/teacher/my-classes`}>Back to classes</Link><TeacherChallengePanel ownerKey={`${user.schoolId}:${user.id}`} games={games} endpoint="/api/v1/apk/challenges" classesEndpoint="/api/v1/apk/challenges/teacher-classes" /></main>;
}
