...
function patchRequest(body: unknown): Request {
  return new Request(`${origin}/api/admin/employees/${accountId}/status`, {
    method: "PATCH",
...
describe("Accounts status administration route", () => {
  beforeEach(() => {
    execute.mockReset();
    session.token = undefined;
  });

  it("denies an anonymous status change before returning data", async () => {
    ...
    const response = await PATCH(
      patchRequest({ status: "SUSPENDED", idempotencyKey: "key-1" }),
      routeContext(),
    );
