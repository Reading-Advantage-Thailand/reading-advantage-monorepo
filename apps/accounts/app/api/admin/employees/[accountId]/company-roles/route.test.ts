...
function putRequest(body: unknown): Request {
  return new Request(`${origin}/api/admin/employees/${accountId}/company-roles`, {
    method: "PUT",
...
describe("Accounts company-roles administration route", () => {
  beforeEach(() => {
    execute.mockReset();
    session.token = undefined;
  });

  it("denies an anonymous company-role replacement before returning data", async () => {
    ...
    const response = await PUT(
      putRequest({ roleKeys: ["EMPLOYEE"], idempotencyKey: "key-1" }),
      routeContext(),
    );
