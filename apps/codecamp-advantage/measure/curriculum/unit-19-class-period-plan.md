# Unit 19 Class Period Plans: Real-World Practice
...
- Every change starts with an Issue and a Measure-style track, then ends with a reviewed PR
...
1. Read the Issue
   ↓
2. Create a Measure-style track with a short `spec.md` and `plan.md`
   ↓
3. Understand the acceptance criteria
   ↓
4. Create a feature branch: feat/issue-1-module-completion
   ↓
5. Implement the change (TDD: write tests first!)
   ↓
6. Run lint, typecheck, and tests locally
   ↓
7. Commit with conventional commit message
   ↓
8. Push the branch
   ↓
9. Open a PR referencing the Issue and Measure track: "Closes #1"
   ↓
...
2. Create a short Measure-style track:
   - `spec.md`: summary, requirements, non-goals, acceptance criteria
   - `plan.md`: Red, Green, review, and acceptance tasks
3. Identify the changes needed:
   ...
   - Write unit tests
4. Estimate the scope — is this one commit or multiple?
5. Plan the implementation order: test → domain → router → UI
...
2. Create the Measure-style track and plan the implementation:
...
## Measure Track
Track: issue-1-module-completion
...
## Testing
- [x] Red test failed before implementation (`pnpm test --filter=@tracker/domain`)
...
1. Read the Issue
2. Create a Measure-style track with `spec.md` and `plan.md`
3. Create a branch
...
- ✅ Control AI-assisted development with Measure specs, plans, and review evidence
...
19 units. 88 class periods. 4 portfolio projects. 1 complete full-stack application from database to deployment.
