...
describe("resolveRequestLocale", () => {
  it("uses a valid English locale cookie", () => {
    ...
    expect(resolveRequestLocale(request)).toEqual({ locale: "en", fromCookie: true });
  });
  ...
  it("uses a valid Thai locale cookie", () => {
    ...
    expect(resolveRequestLocale(request)).toEqual({ locale: "th", fromCookie: true });
  });

  it("defaults a missing cookie", () => {
    const request = new Request("https://codecamp.reading-advantage.com/");

    expect(resolveRequestLocale(request)).toEqual({ locale: "th", fromCookie: false });
  });
