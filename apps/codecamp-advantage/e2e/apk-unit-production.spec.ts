...
const localCheckerResult = {
  ...
  checks: [{ checkId: "manifest.shape", passed: false, evidenceDigest: fakeDigest }, { checkId: "result.behavior", passed: false, evidenceDigest: fakeDigest }, { checkId: "git.clean", passed: false, evidenceDigest: fakeDigest }],
};
...
test.beforeAll(() => {
  ...
  writeFileSync(join(work, "packages/codecamp-knowledge/fixtures/apk-guided/src/game-state.ts"), "export function evaluateAttempt(objectiveId: string, selectedAnswerId: string, correctAnswerId: string, attempts: number) { return { objectiveId, correct: selectedAnswerId === correctAnswerId, attempts }; }\n");
