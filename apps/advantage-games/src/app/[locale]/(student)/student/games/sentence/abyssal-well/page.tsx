...
const AbyssalWellGame = dynamic(
  () =>
    import("@/components/games/sentence/abyssal-well").then(
      (mod) => mod.AbyssalWellGame,
    ),
  { ssr: false },
);
...
export default function AbyssalWellPage() {
    ...
    const fetchSentences = async () => {
      try {
        ...
        const res = await fetch(
          `/api/v1/games/abyssal-well/sentences?locale=${locale}`,
        );
  ...
  const handleComplete = useCallback(
    async (results: { xp: number; accuracy: number }) => {
      ...
      try {
        await fetch("/api/v1/games/abyssal-well/complete", {
          method: "POST",
          headers: {
  ...
  return (
    <main className="min-h-screen px-3 pt-3 pb-6 md:px-6 md:pt-6 transition-colors duration-300 text-slate-900">
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 md:gap-8">
        <Header
          heading="The Abyssal Well"
          text="Defend the rim! Shoot enemies carrying words in the correct order!"
        >
