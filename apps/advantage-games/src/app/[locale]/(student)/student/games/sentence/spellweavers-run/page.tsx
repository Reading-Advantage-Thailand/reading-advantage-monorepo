...
const SpellweaversRunGame = dynamic(
  () =>
    import("@/components/games/sentence/spellweavers-run").then(
      (mod) => mod.SpellweaversRunGame,
    ),
  { ssr: false },
);
...
export default function SpellweaversRunPage() {
    ...
    const fetchSentences = async () => {
      try {
        ...
        const res = await fetch(
          `/api/v1/games/spellweavers-run/sentences?locale=${locale}`,
        );
  ...
  const handleComplete = useCallback(
    async (results: { xp: number; accuracy: number; difficulty: string }) => {
      ...
      try {
        await fetch("/api/v1/games/spellweavers-run/complete", {
          method: "POST",
          headers: {
  ...
  return (
    <main className="min-h-screen px-3 pt-3 pb-6 md:px-6 md:pt-6 transition-colors duration-300 text-slate-900">
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 md:gap-8">
        <Header
          heading="Spellweaver's Run"
          text="Collect word orbs in order to form sentences!"
        >
