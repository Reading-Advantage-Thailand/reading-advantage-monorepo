...
const RuneForgeChamberGame = dynamic(
  () =>
    import("@/components/games/sentence/rune-forge-chamber").then(
      (mod) => mod.RuneForgeChamberGame,
    ),
  { ssr: false },
);
...
export default function RuneForgeChamberPage() {
    ...
    const fetchSentences = async () => {
      try {
        ...
        const res = await fetch(
          `/api/v1/games/rune-forge-chamber/sentences?locale=${locale}`,
        );
  ...
  const handleComplete = useCallback(
    async (results: { xp: number; accuracy: number }) => {
      ...
      try {
        await fetch("/api/v1/games/rune-forge-chamber/complete", {
          method: "POST",
          headers: {
