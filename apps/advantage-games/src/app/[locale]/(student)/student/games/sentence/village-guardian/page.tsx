...
const VillageGuardianGame = dynamic(
  () =>
    import("@/components/games/sentence/village-guardian").then(
      (mod) => mod.VillageGuardianGame,
    ),
  { ssr: false },
);
...
export default function VillageGuardianPage() {
    ...
    const fetchSentences = async () => {
      try {
        ...
        const res = await fetch(
          `/api/v1/games/village-guardian/sentences?locale=${locale}`,
        );
  ...
  const handleComplete = useCallback(
    async (results: { xp: number; accuracy: number }) => {
      ...
      try {
        await fetch("/api/v1/games/village-guardian/complete", {
          method: "POST",
          headers: {
  ...
  return (
    <main className="min-h-screen px-3 pt-3 pb-6 md:px-6 md:pt-6 transition-colors duration-300 text-slate-900">
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 md:gap-8">
        <Header
          heading="Village Guardian"
          text="Defend the village! Rescue villagers in the correct order and lead them to safety!"
        >
