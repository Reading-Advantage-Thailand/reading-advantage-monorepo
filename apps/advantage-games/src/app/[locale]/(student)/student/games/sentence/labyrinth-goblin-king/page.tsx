...
const LabyrinthGoblinKingGame = dynamic(
  () =>
    import("@/components/games/sentence/labyrinth-goblin-king").then(
      (mod) => mod.LabyrinthGoblinKingGame,
    ),
  { ssr: false },
);
...
export default function LabyrinthGoblinKingPage() {
    ...
    const fetchSentences = async () => {
      try {
        ...
        const res = await fetch(
          `/api/v1/games/labyrinth-goblin-king/sentences?locale=${locale}`,
        );
  ...
  const handleComplete = useCallback(
    async (results: { xp: number; accuracy: number }) => {
      ...
      try {
        await fetch("/api/v1/games/labyrinth-goblin-king/complete", {
          method: "POST",
          headers: {
  ...
  return (
    <main className="min-h-screen px-3 pt-3 pb-6 md:px-6 md:pt-6 transition-colors duration-300 text-slate-900">
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 md:gap-8">
        <Header
          heading="Labyrinth of the Goblin King"
          text="Navigate the dungeon maze! Collect word orbs in order and become a Paladin to defeat the goblins!"
        >
