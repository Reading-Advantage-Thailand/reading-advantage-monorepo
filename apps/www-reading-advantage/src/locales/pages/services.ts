export const en = {
  hero: {
    title: "Services",
    subtitle: "How We Deliver Value to Your School",
    description:
      "From comprehensive platforms to fully managed solutions, we offer flexible delivery models tailored to Thai schools' needs.",
    heading: "Our Services",
    subheading: "Solutions for Every School",
  },
  services: [
    {
      name: "Blended Learning Model",
      status: "Launching May 2026",
      statusBadge: "COMING SOON",
      description:
        "Physical workbooks combined with digital app - the best of both worlds for Thai classrooms.",
      features: [
        "15 CEFR levels (A1-C1) available",
        "Sample workbooks available January 2026",
        "Empower Thai teachers to deliver native-standard English",
        "Lesson plans and teacher guides included",
        "2-day onboarding and support",
        "Integrated workbook + app learning system",
      ],
      cta: "Request Information",
      href: "/services/blended-learning",
      image: "/images/blended-learning.png",
    },
    {
      name: "Managed Service",
      status: "Coming 2027",
      statusBadge: "ROADMAP",
      description:
        "The white-glove solution: We provide the technology AND the teacher for complete English department outsourcing.",
      features: [
        "Fully outsourced English department",
        "Structured implementation",
        "Certified Reading Advantage teachers",
        "Complete curriculum and materials",
        "Progress tracking and reporting",
        "Regular quality assurance",
      ],
      cta: "Learn More",
      href: "/services/managed-service",
      image: "/images/teacher-and-dashboard.png",
    },
    {
      name: "Onboarding & Setup",
      status: "Available Now",
      statusBadge: "ACTIVE",
      description:
        "Structured implementation support for your school.",
      features: [
        "Account setup and configuration",
        "Student and teacher enrollment",
        "Technical training for staff",
        "Best practices workshops",
        "Ongoing support resources",
        "Data migration assistance",
      ],
      cta: "Get Started",
      href: "/contact",
      image: "/images/students-at-board.png",
    },
    {
      name: "Free English Reading Assembly",
      status: "Now Booking",
      statusBadge: "ACTIVE",
      description:
        "Bring excitement to your school with our free 1-hour English reading assembly! Students enjoy games, songs, competitions, and prizes while discovering the joy of reading in English.",
      features: [
        "Free - no cost to school",
        "1 hour - we handle everything",
        "Games, songs, competitions",
        "Prizes for everyone",
      ],
      cta: "Schedule Free Activity",
      href: "/contact",
      image: "/images/primary-assembly-activity.png",
    },
  ],
  cta: {
    title: "Ready to Transform Your English Program?",
    description:
      "Contact us to discuss which service model works best for your school.",
    button: "Get in Touch",
  },
};

export const th = {
  hero: {
    title: "บริการ",
    subtitle: "วิธีที่เราส่งมอบมูลค่าให้โรงเรียนของคุณ",
    description:
      "จากแพลตฟอร์มที่ครอบคลุมไปจนถึงโซลูชันที่จัดการได้ครบถ้วน เรามอบรูปแบบการส่งมอบที่ยืดหยุ่นกับความต้องการของโรงเรียนไทย",
    heading: "บริการของเรา",
    subheading: "โซลูชันสำหรับทุกโรงเรียน",
  },
  services: [
    {
      name: "รูปแบบการเรียนแบบผสมผสาน",
      status: "เปิดตัวเดือนพฤษภาคม 2026",
      statusBadge: "เร็วๆ นี้",
      description:
        "หนังสือแบบฝึกหัดทางกายภาพผสมกับแอปดิจิทัล - สิ่งที่ดีที่สุดจากทั้งสองโลกสำหรับห้องเรียนไทย",
      features: [
        "15 ระดับ CEFR (A1-C1) พร้อมใช้งาน",
        "ตัวอย่างหนังสือแบบฝึกหัดพร้อมใช้งานมกราคม 2026",
        "มอบพลังให้ครูไทยสามารถสอนภาษาอังกฤษในระดับมาตรฐานเนทีฟได้",
        "มีแผนการสอนและคู่มือครูพร้อม",
        "การฝึกอบรมเริ่มต้น 2 วันและการสนับสนุน",
        "ระบบการเรียนรู้แบบผสมผสานหนังสือแบบฝึกหัด + แอป",
      ],
      cta: "ขอข้อมูล",
      href: "/services/blended-learning",
      image: "/images/blended-learning.png",
    },
    {
      name: "บริการจัดการแบบครบวงจร",
      status: "มาปี 2027",
      statusBadge: "แผนงาน",
      description:
        "โซลูชันแบบไขมือขาว: เรามอบทั้งเทคโนโลยีและครูสำหรับการจัดการแผนกวิชาภาษาอังกฤษแบบครบวงจรภายนอก",
      features: [
        "แผนกวิชาภาษาอังกฤษแบบจัดการภายนอกทั้งหมด",
        "การเริ่มต้นที่มีโครงสร้าง",
        "ครู Reading Advantage ที่ได้รับการรับรอง",
        "หลักสูตรและวัสดุครบถ้วน",
        "การติดตามความก้าวหน้าและรายงาน",
        "การรับรองคุณภาพอย่างสม่ำเสมอ",
      ],
      cta: "เรียนรู้เพิ่มเติม",
      href: "/services/managed-service",
      image: "/images/teacher-and-dashboard.png",
    },
    {
      name: "การฝึกอบรมและการตั้งค่า",
      status: "พร้อมใช้งานแล้ว",
      statusBadge: "ใช้งาน",
      description:
        "การเริ่มต้นที่มีโครงสร้างสำหรับโรงเรียนของคุณ",
      features: [
        "การตั้งค่าและการกำหนดบัญชี",
        "การลงทะเบียนนักเรียนและครู",
        "การอบรมเทคนิคสำหรับบุคลากร",
        "เวิร์คชอปแนวปฏิบัติที่ดี",
        "แหล่งข้อมูลการสนับสนุนต่อเนื่อง",
        "การช่วยเหลือการย้ายข้อมูล",
      ],
      cta: "เริ่มต้นใช้งาน",
      href: "/contact",
      image: "/images/students-at-board.png",
    },
    {
      name: "กิจกรรมส่งเสริมการอ่านภาษาอังกฤษ\nFree English Reading Assembly",
      status: "เปิดรับจองแล้ว",
      statusBadge: "ใช้งาน",
      description:
        "กิจกรรมส่งเสริมการอ่านภาษาอังกฤษ 1 ชั่วโมง ฟรี! นักเรียนสนุกกับเกม เพลง การแข่งขัน พร้อมรับของรางวัล",
      features: [
        "ฟรี - ไม่มีค่าใช้จ่าย",
        "1 ชั่วโมง - เราจัดให้ทั้งหมด",
        "เกม เพลง การแข่งขัน - นักเรียนสนุกได้ความรู้",
        "ของรางวัลทุกคน - นักเรียนทุกคนได้รับของที่ระลึก",
      ],
      cta: "นัดหมายกิจกรรมฟรี",
      href: "/contact",
      image: "/images/primary-assembly-activity.png",
    },
  ],
  cta: {
    title: "พร้อมที่จะปฏิวัติโปรแกรมภาษาอังกฤษของคุณหรือยัง?",
    description:
      "ติดต่อเราเพื่อหารือเกี่ยวกับรูปแบบบริการใดที่เหมาะสมกับโรงเรียนของคุณ",
    button: "ติดต่อเรา",
  },
};

export const zh = {
  hero: {
    title: "服务",
    subtitle: "我们如何为您的学校创造价值",
    description:
      "从综合平台到完全托管解决方案，我们提供专为泰国学校需求量身定制的灵活交付模式。",
    heading: "我们的服务",
    subheading: "适合每所学校的解决方案",
  },
  services: [
    {
      name: "混合学习模式",
      status: "2026年5月推出",
      statusBadge: "即将推出",
      description:
        "实体练习册结合数字应用程序 - 为泰国课堂提供两全其美的解决方案。",
      features: [
        "15个CEFR级别（A1-C1）可用",
        "2026年1月提供练习册样本",
        "赋能泰国教师提供母语标准英语",
        "包含教案和教师指南",
        "2天入职和支持",
        "集成练习册+应用学习系统",
      ],
      cta: "请求信息",
      href: "/services/blended-learning",
      image: "/images/blended-learning.png",
    },
    {
      name: "托管服务",
      status: "2027年",
      statusBadge: "路线图",
      description:
        "白手套解决方案：我们同时提供技术和教师，实现完整的英语部门外包。",
      features: [
        "完全外包英语部门",
        "结构化实施",
        "经过认证的阅读优势教师",
        "完整的课程和材料",
        "进度跟踪和报告",
        "定期质量保证",
      ],
      cta: "了解更多",
      href: "/services/managed-service",
      image: "/images/teacher-and-dashboard.png",
    },
    {
      name: "入职和设置",
      status: "现在可用",
      statusBadge: "活跃",
      description: "提供结构化实施支持，帮助学校使用 Reading Advantage。",
      features: [
        "账户设置和配置",
        "学生和教师注册",
        "员工技术培训",
        "最佳实践研讨会",
        "持续支持资源",
        "数据迁移协助",
      ],
      cta: "开始使用",
      href: "/contact",
      image: "/images/students-at-board.png",
    },
    {
      name: "英语阅读推广活动\nFree English Reading Assembly",
      status: "现可预订",
      statusBadge: "活跃",
      description:
        "为学校带来活力的免费 1 小时英语阅读集会！学生在游戏、歌曲、竞赛和奖品中体验英语阅读的乐趣。",
      features: [
        "免费 - 学校无需费用",
        "1 小时 - 全程由我们负责",
        "游戏、歌曲、竞赛 - 学生边玩边学",
        "人人有奖 - 每位学生都有纪念品",
      ],
      cta: "预约免费活动",
      href: "/contact",
      image: "/images/primary-assembly-activity.png",
    },
  ],
  cta: {
    title: "准备好变革您的英语课程了吗？",
    description: "联系我们讨论哪种服务模式最适合您的学校。",
    button: "联系我们",
  },
};
