...
const mockHolder = vi.hoisted(() => ({
  generateObject: vi.fn().mockImplementation(() => {
    attemptCount++;
    if (attemptCount <= 2) {
      throw new Error(`[IntegrationFixture] attempt ${attemptCount} failure`);
    }
    return { passed: true, summary: "[IntegrationFixture] LGTM on attempt 3", comments: [] };
  }),
}));
...
describe("Phase 5 — retry then succeed", () => {
  beforeAll(() => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(updatePrReview).mockImplementation(async (args) => {
      ...
      return {
        ...
        userId: "u1",
        prUrl: "https://github.com/org/repo/pull/1",
        reviewStatus: (input.reviewStatus ?? "pending") as "pending" | "approved" | "needs_changes" | "reviewed",
        llmReviewSummary: input.llmReviewSummary ?? null,
        reviewedAt: input.reviewStatus !== "pending" ? new Date() : null,
        createdAt: new Date(),
      };
    });
