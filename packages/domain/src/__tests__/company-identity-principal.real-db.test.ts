...
import { companyProductPrincipals, db, users } from "@reading-advantage/db";

import { resolveSalesCompanyPrincipal } from "../company-identity-principal.js";

const describeRealDatabase =
  process.env.RUN_SALES_MAPPING_REAL_DB_TESTS === "true"
    ? describe
    : describe.skip;
...
describeRealDatabase(
  "Sales company principal mapping (real PostgreSQL)",
  () => {
    it("proves concurrent first login, conflicts, org/role denial, and role synchronization", async () => {
      ...
      expect(concurrent.map((principal) => principal?.user.id)).toEqual([
        localUserId,
        localUserId,
      ]);
      await expect(
        ...
        user: { id: localUserId, role: "SALES_REP" },
      });
      ...
      await expect(
        resolveSalesCompanyPrincipal(db, {
          ...identity,
      ...
      await expect(
        db
          .select({ id: users.id })
          .from(users)
          .where(eq(users.id, occupiedUsernameId)),
      ).resolves.toEqual([{ id: occupiedUsernameId }]);
      ...
      ).resolves.toMatchObject({ user: { id: `sales:${occupiedId}` } });
      await expect(
        db.select({ id: users.id }).from(users).where(eq(users.id, occupiedId)),
      ).resolves.toEqual([{ id: occupiedId }]);
      ...
      await expect(
        resolveSalesCompanyPrincipal(db, identity),
      ).resolves.toMatchObject({ user: { role: "SALES_REP" } });
      const [synchronized] = await db
        .select({
          userRole: users.role,
