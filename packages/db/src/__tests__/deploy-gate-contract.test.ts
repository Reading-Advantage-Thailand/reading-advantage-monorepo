  ...
  for (let i = 1; i < stepBlocks.length; i++) {
    ...
    if (inlineArgsMatch) {
      ...
      if (argsKeyIndex !== -1) {
        ...
        const blockArgMatches = [
          ...afterArgs.matchAll(
            /^\s*-\s*(?:"((?:\\.|[^"])*)"|'((?:''|[^'])*)')\s*$/gm,
          ),
        ];
        args = blockArgMatches
          .map((m) =>
            m[1] !== undefined
              ? m[1].replace(/\\"/g, '"')
              : (m[2] ?? "").replace(/''/g, "'"),
...
describe("Phase 4 — Task 15: FR-4 codecamp deploy gate (cloudbuild.yaml)", () => {
  ...
  it("contains a migrate step that runs `pnpm --filter @reading-advantage/db migrate` before deploy-cloudrun", () => {
    ...
    expect(deployIdx, "deploy-cloudrun step must exist").toBeGreaterThanOrEqual(
      0,
    );
    ...
    const migrateStep = steps.find((s) =>
      s.args.some(
        (a) =>
          a.includes("pnpm") && a.includes("--filter") && a.includes("migrate"),
      ),
    );
    expect(
      migrateStep,
  ...
  it("requires the Codecamp SSO principal migration before candidate deployment", () => {
    ...
    const doctorStep = steps.find((step) =>
      step.args.some(
        (argument) =>
          argument.includes("doctor") && argument.includes("--check"),
      ),
    );
  ...
  it("stages the SSO revision with zero traffic until legacy mappings exist", () => {
    const text = readFileSync(CLOUDBUILD_PATH, "utf8");
    const steps = parseCloudBuildSteps(text);
    const deploy = steps.find((step) => step.id === "deploy-cloudrun");
    expect(deploy).toBeDefined();
    expect(deploy?.args).toContain("--tag=sso-candidate");
  ...
  it("wires CODECAMP_DATABASE_URL through Cloud Build Secret Manager availableSecrets", () => {
    ...
    expect(
      ...
      /versionName:\s*"?projects\/\$PROJECT_ID\/secrets\/CODECAMP_DATABASE_URL\/versions\/latest"?[\s\S]*env:\s*"CODECAMP_DATABASE_URL"/,
    );
    expect(
      text,
...
describe("Phase 4 — Task 15: FR-4 deploy-gate pattern documented in packages/db/README.md", () => {
  ...
  it("documents the deploy-gate pattern with a codecamp reference and DIRECT_DATABASE_URL guidance", () => {
    ...
    expect(text, "README must mention the deploy-gate pattern").toMatch(
      /deploy[- ]?gate/i,
    );
    expect(
      text,
      "README must reference codecamp-advantage as the canonical example",
    ).toMatch(/codecamp[- ]?advantage/);
    expect(
      text,
      "README must call out DIRECT_DATABASE_URL for the gate",
  ...
  it("includes a production reconciliation runbook (doctor report → review → --repair)", () => {
    ...
    expect(
      text,
      "runbook must mention `review` (the manual sign-off step)",
    ).toMatch(/review/i);
  });
});
...
describe("Phase 4 — Task 16: fresh-DB end-to-end shell harness (scripts/ci/fresh-db-e2e.sh)", () => {
  ...
  it("starts Postgres, runs `pnpm migrate`, then runs `pnpm doctor --check` (Phase 4 closeout gate)", () => {
    ...
    expect(
      text,
      "must bring up a postgres container (docker compose or docker run)",
    ).toMatch(/docker\s+(compose|run).*postgres/i);
    expect(
      text,
      "must reset the target database before migration so the gate proves fresh-DB behavior",
    ).toMatch(
      /DROP DATABASE IF EXISTS reading_advantage[\s\S]*CREATE DATABASE reading_advantage/,
    );
    expect(
      text,
      "must export DIRECT_DATABASE_URL before running doctor --check",
    ).toMatch(/export\s+DIRECT_DATABASE_URL=/);
    expect(text, "must run `pnpm migrate` (or the filter form)").toMatch(
      /pnpm[^.\n]*migrate/,
    );
    expect(
      text,
      "must run `pnpm doctor --check` (the closeout gate command)",
    ).toMatch(/doctor[^.\n]*--check|--check[^.\n]*doctor/);
  });
});
...
describe("Phase 4 — Task 17: project memory (tech-debt.md + lessons-learned.md)", () => {
  it("updates the P0 row in measure/tech-debt.md to reflect db-side fix + remaining open scope", () => {
    ...
    expect(row, "P0 row must mention the doctor is available").toMatch(
      /doctor/i,
    );
    expect(
      row,
