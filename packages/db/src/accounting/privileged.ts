...
/**
 * Determines whether a database role has no cluster authority or inherited membership.
 * @param role The catalog projection for a runtime or migration role.
 * @returns Whether the role satisfies the reviewed least-privilege boundary.
 */
...
/**
 * Applies the accounting runtime grants through the migration-owner connection.
 * @param input Migration-owner URL plus the separate runtime and migration roles.
 * @returns A promise that resolves after current and future objects are secured.
 * @throws When roles overlap, inherit authority, violate ownership, or use unsafe names.
 */
export async function configureAccountingDatabasePrivileges(input: {
  ...
  if (input.runtimeRole === input.migrationRole) {
    throw new Error(
      "Accounting runtime and migration database roles must be separate.",
    );
  }
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        "Accounting database roles must be non-inheriting, membership-free, and free of cluster privileges.",
      );
    }
    ...
    await sql.begin(async (tx) => {
      ...
      await tx.unsafe(
        `GRANT SELECT, INSERT ON TABLE public.accounting_submissions TO ${runtimeRole}`,
      );
      await tx.unsafe(
        `REVOKE UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER ON TABLE public.accounting_submissions FROM ${runtimeRole}`,
      );
      await tx.unsafe(
