 ...
 *
 * Each row is one expense or bill submitted by staff for approval. The record
 * is append-only: approval workflows in later phases append new records rather
 * than mutating submissions, so no `updated_at` column exists. Money is stored
 * in exact minor units as digit text — never as a float. `scope_company_id` is
 * deliberately a plain uuid without a foreign key: the owning company account
 * lives in the separate company_identity database, which cross-database
