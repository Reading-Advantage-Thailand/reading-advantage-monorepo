...
/**
 * Enables or disables the reviewed trigger sentinel for a doctor counterexample.
 * @param sql Administrative PostgreSQL connection for the scratch database.
 * @param sentinel The reviewed immutable-trigger sentinel.
 * @param enabled Whether the trigger should be active after the statement.
 * @returns A promise that resolves after the trigger state changes.
 */
async function setDoctorTriggerEnabled(
  sql: ReturnType<typeof postgres>,
  sentinel: CompanyIdentityDoctorSentinel,
  enabled: boolean,
): Promise<void> {
  if (sentinel.kind !== "trigger" || !sentinel.triggerName) {
    throw new Error(
      "Doctor trigger toggle requires a complete trigger sentinel.",
    );
  }
  const schemaName = quoteCatalogIdentifier(sentinel.schemaName);
  const tableName = quoteCatalogIdentifier(sentinel.tableName);
  const triggerName = quoteCatalogIdentifier(sentinel.triggerName);
  await sql.unsafe(
    `ALTER TABLE ${schemaName}.${tableName} ${enabled ? "ENABLE" : "DISABLE"} TRIGGER ${triggerName}`,
  );
}
  ...
  it(
    ...
    async () => {
      await withCompanyIdentityScratchDatabase(
        async ({ adminSql, databaseName, directDatabaseUrl }) => {
        ...
        `;
          ...
          `;
            ...
            if (!latestLedger) {
              throw new Error(
                "Current identity ledger unexpectedly has no rows.",
              );
            }
            await adminSql`
          ...
          `;
            ...
            if (latestSentinel.kind === "trigger") {
              ...
              try {
                const disabledTriggerInspection =
                  await doctor.inspectCompanyIdentityDatabase({
                    directDatabaseUrl,
                  });
                expect(disabledTriggerInspection.clean).toBe(false);
                expect(
                  disabledTriggerInspection.issues.map(({ code }) => code),
                ).toContain("MISSING_SENTINEL");
              } finally {
                await setDoctorTriggerEnabled(adminSql, latestSentinel, true);
              }

              const [functionRow] = await adminSql<
                Array<{ definition: string }>
              >`
                select pg_catalog.pg_get_functiondef(trigger.tgfoid) as definition
                  from pg_catalog.pg_trigger trigger
                 where trigger.tgname = ${latestSentinel.triggerName!}
                   and not trigger.tgisinternal
              `;
              await adminSql.unsafe(`
                create or replace function public.company_identity_reject_audit_mutation()
                returns trigger
                language plpgsql
                as $$ begin return null; end; $$
              `);
              try {
                const modifiedFunctionInspection =
                  await doctor.inspectCompanyIdentityDatabase({
                    directDatabaseUrl,
                  });
                expect(modifiedFunctionInspection.clean).toBe(false);
                expect(
                  modifiedFunctionInspection.issues.map(({ code }) => code),
                ).toContain("MISSING_SENTINEL");
              } finally {
                await adminSql.unsafe(functionRow!.definition);
              }
            }
