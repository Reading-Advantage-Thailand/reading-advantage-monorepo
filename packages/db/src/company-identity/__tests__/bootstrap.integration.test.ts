...
interface BootstrapModule {
  bootstrapCompanyIdentity(input: { directDatabaseUrl: string }): Promise<void>;
}
  ...
  it("replays stable organization and application records without duplicate grants or evidence", async () => {
    await withCompanyIdentityScratchDatabase(
      async ({ adminSql, directDatabaseUrl }) => {
        expect(
          await adminSql`select current_setting('server_version_num')::int as version`,
        ).toHaveLength(1);
        ...
        `;
          expect(
            firstRoleDefinitions.map(
              ({ stable_key: stableKey, role_key: roleKey }) =>
                `${stableKey}:${roleKey}`,
            ),
          ).toEqual([
            "marketing:ADMIN",
        ...
        `;
          expect(
            evidenceCountsBeforeReplay[0]!.audit_events,
          ).toBeLessThanOrEqual(1);
          expect(
            evidenceCountsBeforeReplay[0]!.idempotency_records,
          ).toBeLessThanOrEqual(1);
        ...
        `;
          ...
          `,
            ...
            { display_name: "Operator Company", status: "SUSPENDED" },
          ]);
          expect(
            await sql`
            select display_name, status
          ...
          `,
            ...
            { display_name: "Operator Marketing", status: "DISABLED" },
          ]);
          expect(
            await sql`
            select a.stable_key, r.role_key, r.description, r.status
