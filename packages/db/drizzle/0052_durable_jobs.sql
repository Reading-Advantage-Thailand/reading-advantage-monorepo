...
CREATE TABLE "durable_jobs" (
	...
	CONSTRAINT "durable_jobs_queue_name_check" CHECK (char_length("queue_name") BETWEEN 1 AND 100 AND char_length(btrim("queue_name")) BETWEEN 1 AND 100 AND "queue_name" = btrim("queue_name") AND "queue_name" ~ '^[a-z][a-z0-9]*(?:[._-][a-z0-9]+)*$'),
	...
	CONSTRAINT "durable_jobs_rerun_tuple_check" CHECK ((("rerun_queue_name" IS NULL AND "rerun_payload_json" IS NULL AND "rerun_payload_fingerprint" IS NULL AND "rerun_max_attempts" IS NULL AND "rerun_available_at" IS NULL) OR ("rerun_queue_name" IS NOT NULL AND "rerun_payload_json" IS NOT NULL AND "rerun_payload_fingerprint" IS NOT NULL AND "rerun_max_attempts" BETWEEN 1 AND 1000 AND "rerun_available_at" IS NOT NULL))),
	...
	CONSTRAINT "durable_jobs_redelivery_state_check" CHECK (
		("redeliver_current_attempt" = true AND "state" = 'pending' AND "attempt" >= 1 AND "attempt" <= "max_attempts")
		OR ("redeliver_current_attempt" = false AND ("state" <> 'pending' OR "attempt" < "max_attempts"))
	),
