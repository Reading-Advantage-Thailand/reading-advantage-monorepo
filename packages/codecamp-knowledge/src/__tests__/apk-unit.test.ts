...
describe("Codecamp APK game-creation unit", () => {
  ...
  it("ships distinct guided and independent repository fixtures", async () => {
    ...
    await expect(runTutorialStep(executableManifest, "wedo.apk.manifest", { readAllowedFile: async (path) => path.endsWith("game-state.ts") ? guidedResult : guided, runAllowedCommand: async () => "", now: () => "2026-07-11T00:00:00Z" })).resolves.toMatchObject({ passed: false, checks: [{ checkId: "manifest.shape", passed: false }, { checkId: "result.behavior", passed: false }, { checkId: "git.clean", passed: true }] });
    const abiInvalid = "export const cartridgeManifest = { id: true, title: false, description: 1, version: 2, runtimeApiVersion: 3, inputMode: 'invalid', requiredAssetBindings: { wrong: true }, capabilities: `wrong` };";
    await expect(runTutorialStep(executableManifest, "wedo.apk.manifest", { readAllowedFile: async (path) => path.endsWith("game-state.ts") ? "export const educationalResult = { objectiveId: 'fixed', correct: false, attempts: 1 };" : abiInvalid, runAllowedCommand: async () => "", now: () => "2026-07-11T00:00:00Z" })).resolves.toMatchObject({ passed: false, checks: [{ checkId: "manifest.shape", passed: false }, { checkId: "result.behavior", passed: false }, { checkId: "git.clean", passed: true }] });
    const incompatibleRuntime = "export const cartridgeManifest = { id: 'apk.test', title: 'Test', description: 'Test cartridge', version: '1.0.0', runtimeApiVersion: '99.0.0', inputMode: 'vocabulary', requiredAssetBindings: ['background'], capabilities: [] } as const;";
    const completedResult = "export function evaluateAttempt(objectiveId: string, selectedAnswerId: string, correctAnswerId: string, attempts: number) { return { objectiveId, correct: selectedAnswerId === correctAnswerId, attempts }; }";
    await expect(runTutorialStep(executableManifest, "wedo.apk.manifest", { readAllowedFile: async (path) => path.endsWith("game-state.ts") ? completedResult : incompatibleRuntime, runAllowedCommand: async () => "", now: () => "2026-07-11T00:00:00Z" })).resolves.toMatchObject({ passed: false, checks: [{ checkId: "manifest.shape", passed: false }, { checkId: "result.behavior", passed: true }, { checkId: "git.clean", passed: true }] });
  });
