...
export function createLabyrinthGoblinKingCartridge(): StandardExperienceCartridge {
  let active: LabyrinthGoblinKingController | undefined; const standardExperience = createCartridgeStandardExperience({ id: LABYRINTH_GOBLIN_KING_ID, title: "Labyrinth of the Goblin King", description: "Navigate a maze, collect ordered English words, and turn the goblins' pursuit against them.", inputMode: "sentence", objective: "Collect each English sentence word for the Thai prompt.", mechanicInstruction: "Queue turns through the maze and collect word orbs in order.", keyboardKeys: ["W", "A", "S", "D", "Arrow keys"], executeTutorialAction: (id) => active?.queueDirection(id === "action:select-correct" ? "right" : "down") });
  return { manifest: { id: LABYRINTH_GOBLIN_KING_ID, title: "Labyrinth of the Goblin King", description: "Navigate a maze, collect ordered English words, and turn the goblins' pursuit against them.", runtimeApiVersion: "1.0.0", inputMode: "sentence", requiredAssetBindings: ["labyrinth-goblin-king/player"], capabilities: ["capability:input-action-normalization", "capability:language-target-progression", "capability:nonempty-content-precondition", "capability:result-accounting", "capability:single-completion-emission", "capability:time-and-frame-loop"] }, standardExperience, createGameConfig(context) { const input = sentenceInputSchema.parse(context.input); let controller!: LabyrinthGoblinKingController; controller = createLabyrinthGoblinKingController(input, (result) => context.complete(result, controller.snapshot().phase as GameTerminalOutcome), context.seed ?? 0); active = controller; return { width: 390, height: 700, render: { antialias: false, pixelArt: true }, scene: createScene(context, controller) }; } };
}
