...
/** Traversal residual host-proof titles (Dragon Rider has a dedicated dual-host module). */
export const LEGACY_TRAVERSAL_HOST_PROOF_IDS = Object.freeze([
  "spellweavers-run",
  "shadow-gate-dungeon",
...
export type LegacyTraversalHostProofId = (typeof LEGACY_TRAVERSAL_HOST_PROOF_IDS)[number];

export const LEGACY_TRAVERSAL_HOST_PROOF_ASSET_BINDINGS = Object.freeze([
  "effects/32x32/combat/hit-01",
  "top-down/32x32/characters/hero-01",
] as const);

const TITLES: Readonly<Record<LegacyTraversalHostProofId, string>> = Object.freeze({
  "spellweavers-run": "Spellweaver's Run",
  "shadow-gate-dungeon": "Shadow Gate Dungeon",
  "labyrinth-goblin-king": "Labyrinth of the Goblin King",
  "griffin-riders-escape": "Griffin Rider's Escape",
});
...
/**
 * Loads one residual traversal host-proof cartridge by public id.
 * @param cartridgeId Traversal title identifier.
 * @returns Runtime cartridge with single-completion emission.
 */
export async function loadLegacyTraversalHostProofCartridge(
  cartridgeId: string,
): Promise<RuntimeCartridge> {
  if (!(LEGACY_TRAVERSAL_HOST_PROOF_IDS as readonly string[]).includes(cartridgeId)) {
    throw new Error(`Unknown legacy-traversal host-proof cartridge: ${cartridgeId}`);
  }
  const id = cartridgeId as LegacyTraversalHostProofId;
  const title = TITLES[id];
  const definition = Object.freeze({
    id,
    title,
    description: `${title} host-proof gate matching vocabulary translation.`,
    requiredAssetBindings: LEGACY_TRAVERSAL_HOST_PROOF_ASSET_BINDINGS,
          ...
          update(this: GateScene): void {
            ...
            if (choice !== undefined) {
              ...
              context.diagnostic({
                ...
                message: `${title} gate`,
                details: { kind: "choose-gate", gate: choice ? "right" : "left", elapsedMs: Math.max(0, Date.now() - startedAt) },
              });
              render(this);
            }
            if (state.attempts > 0 && launch) {
              ...
              context.diagnostic({
                level: "info",
                code: definition.diagnosticCode,
                message: `${title} launch`,
                details: { kind: "launch", elapsedMs: Math.max(0, Date.now() - startedAt) },
              });
