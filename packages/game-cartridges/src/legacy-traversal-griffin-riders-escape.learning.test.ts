...
describe("legacy traversal Phase 3 Griffin Rider's Escape learning contract", () => {
  it("requires ordered gate collection to advance educational targets and emit one result", async () => {
    ...
    await requireCartridge(
      ...
      "the cartridge must preserve ordered gate progress, exact result accounting, and one completion",
    );
  });
});
