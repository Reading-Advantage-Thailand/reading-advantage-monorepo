...
describe("W4 dual-edition arena cartridges", () => {
  it.each(arenaWaveBlueprints)("loads $id with its frozen manifest and shared scene", async (blueprint) => {
    ...
    expect(cartridge.manifest).toMatchObject({ id: blueprint.id, inputMode: blueprint.inputMode, capabilities: blueprint.mechanic === "patrol-minimap" ? ["arcade-physics", "camera", "tweens"] : ["arcade-physics", "tweens"], requiredAssetSlots: blueprint.requiredAssetSlots });
