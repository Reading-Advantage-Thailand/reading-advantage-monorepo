...
describe("legacy traversal Phase 3 Shadow Gate Dungeon learning contract", () => {
  it("requires ordered crystal collection to advance educational targets and emit one result", async () => {
    ...
    await requireCartridge(
      ...
      "the cartridge must preserve ordered crystal progress, exact result accounting, and one completion",
    );
  });
});
