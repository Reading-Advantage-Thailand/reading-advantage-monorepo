...
describe("activity tRPC adapter", () => {
  it("derives learner and tenant identity from authenticated context", async () => {
    ...
    const activityRouter = createActivityRouter(() => ({ start, append: vi.fn(), get: vi.fn(), assessCheckpoint: vi.fn(), assessTutorial: vi.fn(), prepareTutorial: vi.fn(), reportTutorial: vi.fn(), getTeacherSummary: vi.fn() }));
  ...
  it("exposes teacher summaries only to school-scoped educators", async () => {
    ...
    const activityRouter = createActivityRouter(() => ({ start: vi.fn(), append: vi.fn(), get: vi.fn(), assessCheckpoint: vi.fn(), assessTutorial: vi.fn(), prepareTutorial: vi.fn(), reportTutorial: vi.fn(), getTeacherSummary }));
  ...
  it("derives tutorial report ownership from the authenticated context", async () => {
    ...
    const activityRouter = createActivityRouter(() => ({ start: vi.fn(), append: vi.fn(), get: vi.fn(), assessCheckpoint: vi.fn(), assessTutorial: vi.fn(), prepareTutorial: vi.fn(), reportTutorial, getTeacherSummary: vi.fn() }));
  ...
  it("derives repository-capture ownership before issuing a tutorial credential", async () => {
    ...
    const activityRouter = createActivityRouter(() => ({ start: vi.fn(), append: vi.fn(), get: vi.fn(), assessCheckpoint: vi.fn(), assessTutorial: vi.fn(), prepareTutorial, reportTutorial: vi.fn(), getTeacherSummary: vi.fn() }));
    const caller = activityRouter.createCaller({ auth: { user: { id: "learner-1" }, tenant: { schoolId: null } }, db: {}, tenantDb: {} } as never);
