...
export const INVALID_CAPABILITY_COMBINATIONS = [
  ...
  { id: "authorization.parameter-projection-mismatch", phase: "registration", target: "descriptor", condition: "A named authorization policy requires a parameter projection that the descriptor omits or references with a different projector ID, schema identity, or allowed-key set.", rationale: "Authorization must receive only parameters proven against the exact reviewed policy contract." },
  ...
  { id: "idempotency.audited-mutation-required", phase: "registration", target: "descriptor", condition: "A command or job requiring post-commit audit evidence declares idempotency mode none.", rationale: "Durable terminal settlement prevents committed mutation work from being repeated when post-commit audit projection or append fails." },
  ...
  { id: "registry.reference-identity-mismatch", phase: "registration", target: "registry", condition: "A reference registry returns a policy, resolver, or projector whose stable ID, mode, owner, schema identity, or allowed-key set differs from the exact requested definition.", rationale: "Lookup success cannot substitute a different reviewed definition for the descriptor reference." },
  ...
  { id: "tenancy.global-policy-owner-mismatch", phase: "registration", target: "descriptor", condition: "A global tenancy policy is registered to a package other than the descriptor owner package.", rationale: "Global access approval cannot be borrowed across package ownership boundaries." },
  ...
  { id: "tenancy.referential-owner-policy-required", phase: "registration", target: "descriptor", condition: "Referential tenancy omits or cannot resolve its trusted resolver or owner-scope policy.", rationale: "Referential tables require a verified owner-FK scope chain." },
  ...
  { id: "transaction.external-call-protocol-contradiction", phase: "registration", target: "descriptor", condition: "A transaction forbidding external calls nevertheless declares an external-call protocol reference.", rationale: "Transaction effects must have one unambiguous policy." },
  { id: "transaction.external-call-protocol-unresolved", phase: "registration", target: "descriptor", condition: "A transaction documents external calls through a protocol reference that is not registered.", rationale: "Documented external work requires a reviewed, resolvable protocol." },
