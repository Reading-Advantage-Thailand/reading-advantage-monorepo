# Rune Forge Chamber Mechanic Blueprint

## Identity and fantasy

Forge a rune before the chamber cools. Canonical ID: `game:rune-forge-chamber`; confidence: **high**.

## Learning loop

The player must tap word circles in sentence order. Correct input advances one content step; incorrect or out-of-order input records feedback and an attempt without silently skipping required content.

## World and controls

radial pointer/touch sequencing around a forge. The Phaser rebuild may choose native physics, cameras, pooling, and scene composition while preserving recognizable agency and target readability.

## Progression and terminal state

complete the ordered sequence before the forge timer expires. Score, accuracy, correct answers, and attempts map to the established `GameResults`; authoritative XP and persistence remain host-owned.
