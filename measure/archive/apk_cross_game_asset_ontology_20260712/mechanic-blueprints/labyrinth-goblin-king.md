# Labyrinth of the Goblin King Mechanic Blueprint

## Identity and fantasy

Become a paladin inside a goblin maze. Canonical ID: `game:labyrinth-goblin-king`; confidence: **high**.

## Learning loop

The player must collect ordered word orbs then confront goblins. Correct input advances one content step; incorrect or out-of-order input records feedback and an attempt without silently skipping required content.

## World and controls

maze navigation with camera, collisions, and empowered-state transition. The Phaser rebuild may choose native physics, cameras, pooling, and scene composition while preserving recognizable agency and target readability.

## Progression and terminal state

complete the sentence and final encounter or lose health/time. Score, accuracy, correct answers, and attempts map to the established `GameResults`; authoritative XP and persistence remain host-owned.
