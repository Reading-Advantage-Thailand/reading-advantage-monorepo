# Babel Architect Mechanic Blueprint

## Identity and fantasy

Build a tower from correctly ordered words. Canonical ID: `game:babel-architect`; confidence: **medium**.

## Learning loop

The player must place or type sentence pieces in construction order. Correct input advances one content step; incorrect or out-of-order input records feedback and an attempt without silently skipping required content.

## World and controls

provisional keyboard/pointer construction concept. The Phaser rebuild may choose native physics, cameras, pooling, and scene composition while preserving recognizable agency and target readability.

## Progression and terminal state

complete the sentence structure; exact failure loop remains product-owner provisional. Score, accuracy, correct answers, and attempts map to the established `GameResults`; authoritative XP and persistence remain host-owned.
