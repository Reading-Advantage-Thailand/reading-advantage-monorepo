# Griffin Sky-Joust Mechanic Blueprint

## Identity and fantasy

Joust through the sky on a griffin. Canonical ID: `game:griffin-sky-joust`; confidence: **medium**.

## Learning loop

The player must strike enemies carrying the next sentence word. Correct input advances one content step; incorrect or out-of-order input records feedback and an attempt without silently skipping required content.

## World and controls

tap/key flap with horizontal drift and aerial collision. The Phaser rebuild may choose native physics, cameras, pooling, and scene composition while preserving recognizable agency and target readability.

## Progression and terminal state

complete the ordered targets or lose aerial health. Score, accuracy, correct answers, and attempts map to the established `GameResults`; authoritative XP and persistence remain host-owned.
