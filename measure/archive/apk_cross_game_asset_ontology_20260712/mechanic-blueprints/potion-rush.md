# Potion Rush Mechanic Blueprint

## Identity and fantasy

Serve a busy magical potion shop. Canonical ID: `game:potion-rush`; confidence: **high**.

## Learning loop

The player must collect conveyor ingredients in sentence order for each customer. Correct input advances one content step; incorrect or out-of-order input records feedback and an attempt without silently skipping required content.

## World and controls

tap/drag conveyor and cauldron stations. The Phaser rebuild may choose native physics, cameras, pooling, and scene composition while preserving recognizable agency and target readability.

## Progression and terminal state

serve the order queue before patience/time expires. Score, accuracy, correct answers, and attempts map to the established `GameResults`; authoritative XP and persistence remain host-owned.
