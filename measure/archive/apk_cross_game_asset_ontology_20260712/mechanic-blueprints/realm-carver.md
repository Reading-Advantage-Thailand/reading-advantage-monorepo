# Realm Carver Mechanic Blueprint

## Identity and fantasy

Carve safe territory through wild magic. Canonical ID: `game:realm-carver`; confidence: **medium**.

## Learning loop

The player must capture word beacons in sentence order. Correct input advances one content step; incorrect or out-of-order input records feedback and an attempt without silently skipping required content.

## World and controls

directional movement over a territory grid with camera/minimap. The Phaser rebuild may choose native physics, cameras, pooling, and scene composition while preserving recognizable agency and target readability.

## Progression and terminal state

complete the ordered territory path or lose to corruption/time. Score, accuracy, correct answers, and attempts map to the established `GameResults`; authoritative XP and persistence remain host-owned.
