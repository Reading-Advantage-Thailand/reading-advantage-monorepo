# Wizard vs Zombie Mechanic Blueprint

## Identity and fantasy

Survive a non-graphic magical horde. Canonical ID: `game:wizard-vs-zombie`; confidence: **high**.

## Learning loop

The player must collect the correct vocabulary orb while evading enemies. Correct input advances one content step; incorrect or out-of-order input records feedback and an attempt without silently skipping required content.

## World and controls

free-roam arena movement with camera and indicators. The Phaser rebuild may choose native physics, cameras, pooling, and scene composition while preserving recognizable agency and target readability.

## Progression and terminal state

finish the vocabulary set or lose all health. Score, accuracy, correct answers, and attempts map to the established `GameResults`; authoritative XP and persistence remain host-owned.
