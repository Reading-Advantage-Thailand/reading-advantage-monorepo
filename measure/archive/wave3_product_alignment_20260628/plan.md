...
- [x] Task: Freeze a claims evidence matrix for public website pages. — `phase-0-claims-matrix.md` (commit `8a47d2df`; 30 claim rows CC-01..CC-30, HEAD-confirmed file:line evidence)
...
- [x] Task: Freeze an Advantage Games import policy: standalone-only, pilot import, or full import path. — `phase-0-decisions.md` Decision 3 (commit `8a47d2df`)
...
- [x] Task: Write Red test showing hardcoded `/en/` navigation breaks host embedding. — `36d62c43`
  - Evidence refs: Advantage Games D-07/D-09; executive summary i18n/import-contract gaps.
  - Delivered:
    - `apps/advantage-games/src/__tests__/import-harness/haunted-library-import.test.tsx` — groups 5A/5B/5C/5E; fails at baseline because `@/locales/GamesLocaleContext` and `@/lib/games-runtime` do not exist.
    - `apps/advantage-games/src/components/games/sentence/haunted-library/HauntedLibraryGame.test.tsx` — extended with group 5D (canonical `@/lib/games-runtime` import + `onNavigate` wiring); 2 new tests fail at baseline.
    - `apps/advantage-games/src/app/[locale]/(student)/student/games/vocabulary/dragon-rider/page.test.tsx` — rewritten group 5D; fails at baseline on `/en/student/games` href and missing `onNavigate` call.
    - `apps/advantage-games/src/lib/gameCards.test.ts` — group 5E; fails at baseline with 29 `/en/-prefixed` hrefs.
  - Red command evidence:
    - `pnpm --filter vocabulary-games test --testPathPatterns=import-harness` → fails: `Cannot find module '../../locales/GamesLocaleContext'`.
    - `pnpm --filter vocabulary-games test --testPathPatterns=HauntedLibraryGame` → `Tests: 2 failed, 12 passed` (legacy-dpad renders; no Exit button).
    - `pnpm --filter vocabulary-games test --testPathPatterns=dragon-rider/page` → `Tests: 2 failed, 1 passed` (`/en/student/games` href present; onNavigate not called).
...
- [x] Task: Run Measure phase acceptance and archive the track. — Phase 6 final-acceptance audit at `audit/phase-6-final-acceptance.json` (commit `d5181776`); archive move and closeout manifest committed at `60d7b5c4` (closeout role).
