...
def select_scene_link(joins: dict[str, Any]) -> tuple[dict[str, Any], int, dict[str, Any]]:
    """Selects a deterministic scene link whose four responsive cells can be retained.

    Args:
        joins: Parsed producer joins document.

    Returns:
        The owning record, link index, and selected scene link.

    Raises:
        ValueError: When no exact accepted scene link with four cells exists.
    """
    records = joins.get("records")
    if not isinstance(records, list):
        raise ValueError("fixture-precondition-not-met: records unavailable")
    scene_links = sorted(
        (
            (record, index, link)
            for record in records
            if isinstance(record, dict) and isinstance(record.get("usage_links"), list)
            for index, link in enumerate(record["usage_links"])
            if isinstance(link, dict)
            and isinstance(link.get("normalized_usage"), dict)
            and link["normalized_usage"].get("surface_kind") == "scene"
            and isinstance(link.get("responsive_assessment_cells"), list)
            and len(link["responsive_assessment_cells"]) == 4
        ),
        key=lambda item: (
            str(item[0].get("canonical_path")),
            item[1],
            str(item[2].get("usage_id")),
        ),
    )
    if not scene_links:
        raise ValueError("fixture-precondition-not-met: no exact accepted scene link with four cells")
    return scene_links[0]
...
def apply_fixture(joins: dict[str, Any], rollup: dict[str, Any], mapper: dict[str, Any], operation: str) -> None:
    ...
    """
    if operation == "non_scene_cells":
        record, link_index, link = select_scene_link(joins)
    else:
...
def main() -> int:
    ...
    verifier_path = TRACK / "phase4_v2_batch_schema_contract_test_v2.py"
    payload = {
        "schema_version": "apk-asset-forensics.phase4-v2-batch-schema-contract-test-report.v2",
