...
STANDARD_MAPPER_FINAL_STATUS = "pending_truth_test_review_root"
...
def build_frozen_facts(root: Path, batch: str, errors: list[str]) -> dict[str, Any] | None:
    ...
    provenance = unique_index(errors, require_list(errors, provenance_audit.get("records"), "canonical_path", "provenance.records"), "canonical_path", "provenance.records")
...
def validate_rollup(errors: list[str], rollup: Any, records: list[Any], facts: dict[str, Any], batch: str, joins_path: Path, joins_sha256: str) -> None:
    ...
    for group_id, members in grouped.items():
        ...
        if set(member_paths) != expected_paths or len(member_paths) != len(expected_paths):
        ...
        expected_counts = {
            ...
            "disposition_paths": dict(Counter(item["value"] for item in expected_dispositions.values())),
        }
...
def fixture_hashes(root: Path, errors: list[str]) -> dict[str, str]:
    ...
    try:
        ...
        paths = []
    if len(paths) != len(FIXTURE_OPERATIONS):
