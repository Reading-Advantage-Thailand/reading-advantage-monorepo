...
def validate_fixture_gate(manifest_hash: str | None, manifest: Any, inventory: dict[str, dict[str, Any]] | None, preflight_errors: list[str] | None = None) -> list[str]:
    """Returns structured errors for a complete fixture-gate state.

    Args:
        manifest_hash: Current manifest byte hash, or None when it was unreadable.
        manifest: Parsed manifest, or None when parsing failed.
        inventory: Parsed live fixture inventory, or None when it was unreadable.
        preflight_errors: Structured errors captured while reading external files.

    Returns:
        Deterministic fixture-manifest-binding errors without raising exceptions.
    """
    errors = list(preflight_errors or [])
    if manifest_hash != EXPECTED_FIXTURE_MANIFEST_SHA256:
        error(errors, "fixture-manifest-binding", "manifest byte digest")
    if manifest is None:
        error(errors, "fixture-manifest-binding", "manifest unavailable")
    if inventory is None:
        error(errors, "fixture-manifest-binding", "fixture inventory unavailable")
    if manifest is not None and inventory is not None:
        errors.extend(validate_fixture_set(manifest, inventory))
    return errors


def load_fixture_gate(root: Path) -> tuple[str | None, Any | None, dict[str, dict[str, Any]] | None, list[str]]:
    """Loads fixture-gate inputs while converting all external failures to errors.

    Args:
        root: Repository root containing the manifest and fixture directory.

    Returns:
        Manifest hash, parsed manifest, fixture inventory, and structured errors.
    """
    ...
    try:
        ...
        error(errors, "fixture-manifest-binding", "manifest unreadable")
    try:
        manifest = load(manifest_path)
    except (OSError, UnicodeDecodeError, json.JSONDecodeError):
        error(errors, "fixture-manifest-binding", "manifest invalid JSON")
    try:
        inventory = live_fixture_inventory(root)
    except (OSError, UnicodeDecodeError, json.JSONDecodeError):
        error(errors, "fixture-manifest-binding", "fixture unreadable or invalid JSON")
    return manifest_hash, manifest, inventory, validate_fixture_gate(manifest_hash, manifest, inventory, errors)
...
def run_fixture_gate_orchestration_meta_counterexamples(manifest: dict[str, Any], inventory: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    """Proves unavailable fixture-gate inputs return errors instead of exceptions.

    Args:
        manifest: Valid immutable fixture manifest.
        inventory: Valid live fixture inventory to clone for path mutations.

    Returns:
        Four structured-failure results for manifest and required-fixture failures.
    """
    target = str(FIXTURES / "duplicate-caller-locator-id.json")
    renamed = str(FIXTURES / "renamed-duplicate-caller-locator-id.json")
    cases = [
        ("fixture_gate_missing_manifest", None, None, None, ["fixture-manifest-binding: manifest unreadable"]),
        ("fixture_gate_malformed_manifest", EXPECTED_FIXTURE_MANIFEST_SHA256, None, None, ["fixture-manifest-binding: manifest invalid JSON"]),
        ("fixture_gate_missing_required_fixture", EXPECTED_FIXTURE_MANIFEST_SHA256, manifest, {key: value for key, value in inventory.items() if key != target}, []),
        ("fixture_gate_renamed_required_fixture", EXPECTED_FIXTURE_MANIFEST_SHA256, manifest, {**{key: value for key, value in inventory.items() if key != target}, renamed: inventory[target]}, []),
    ]
    results: list[dict[str, Any]] = []
    for operation, manifest_hash, current_manifest, current_inventory, preflight in cases:
        errors = validate_fixture_gate(manifest_hash, current_manifest, current_inventory, preflight)
        codes = sorted({item.split(":", 1)[0] for item in errors})
        results.append({"fixture": "in-memory-fixture-gate", "operation": operation, "error_codes": codes, "rejected": "fixture-manifest-binding" in codes, "structured_failure": True})
    return results
...
def report(root: Path) -> dict[str, Any]:
    ...
    """
    manifest_hash, manifest, inventory, manifest_errors = load_fixture_gate(root)
    fixture_manifest = {
        "path": str(FIXTURE_MANIFEST),
        "sha256": manifest_hash,
        "fixture_set_sha256": fixture_set_digest(inventory) if inventory is not None else None,
        "behavior_fixture_count": len(inventory) if inventory is not None else None,
        "meta_counterexample_count": 0,
    }
    if manifest_errors:
        behavior: list[dict[str, Any]] = []
        meta: list[dict[str, Any]] = []
        errors, measured = manifest_errors, {}
        frozen_input_hashes: dict[str, str] = {}
    else:
        assert isinstance(manifest, dict) and inventory is not None
        behavior = run_fixtures(root, manifest, inventory)
        meta = run_fixture_set_meta_counterexamples(manifest, inventory) + run_fixture_gate_orchestration_meta_counterexamples(manifest, inventory)
    ...
    fixture_manifest["meta_counterexample_count"] = len(meta)
    ...
    return {
        ...
        "final_status": "truth-test-pass-pending-fresh-v4-independent-global-review-and-root-acceptance" if not errors and all(item["rejected"] for item in counterexamples) else "truth-test-failed",
    }
