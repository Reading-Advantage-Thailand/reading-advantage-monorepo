#!/usr/bin/env python3
"""Verifies Phase 2 v6 finding specificity and residual semantic routing."""
...
DISPATCH_PATH = "phase2-role-dispatch-v6.json"
DISPATCH_SHA256 = "62fb22f1e81f142cc2b60cde455304b21d4c9dcd368e70f5fa235573dd13c60f"
MAPPER_OUTPUTS = (
...
FIXTURE_MANIFEST = "phase2-v6-fixture-manifest.json"
...
def _truth_contract(track_root: Path) -> dict[str, str]:
    ...
    paths = (
        "phase2_v6_truth_verifier.py",
        "phase2_v6_truth_verifier_test.py",
        FIXTURE_MANIFEST,
        "phase2-v6-red-report.json",
    )
...
def _verify_inputs(
    ...
    if (
        dispatch.get("status") != "active-independent-review-remediation"
        or dispatch.get("schema_version") != "apk-t9-phase2-role-dispatch.v6"
    ):
...
def _validate_bundle(
    ...
    *,
    ...
    v5._shape(findings, receipt, v5.RECEIPT_KEYS)
    if (
        receipt.get("owner_role") != "capability-mapper"
        or receipt.get("task_id") != "phase2-specificity-mapper-v4"
...
def verify_phase2(
    ...
    if fixture_path is not None:
        try:
            ...
            if fixture.get("schema_version", "").startswith("apk-t9-phase2-v6"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        json.dumps(
            {
                "schema_version": "apk-t9-phase2-v6-verification-result.v1",
                "state": result.state,
