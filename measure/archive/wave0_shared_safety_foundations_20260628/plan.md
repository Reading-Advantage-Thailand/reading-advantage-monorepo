...
- [x] Task: Read roadmap artifacts: `deduplicated-findings.md`, `critical-high-remediation-plan.md`, `migration-roadmap.md`, and `test-strategy-roadmap.md` [e0ab7241]
...
- [x] Task: Run `build-graph stats ./graph.db` if graph is fresh; record graph status in this plan [e0ab7241]
...
- [x] Task: Inspect `packages/domain/src/tenant-registry.ts`, `tenant-coverage.test.ts`, `packages/api/src/context.ts`, `packages/types`, and auth role schema surfaces [e0ab7241]
...
- [x] Task: Record the current failing/passing baseline for the required verification commands [e0ab7241]
...
- [x] Task: Write Red tests proving all exported Drizzle tables are classified as FLAT, REFERENTIAL, or EXEMPT [42bbbc88]
...
- [x] Task: Write Red tests proving null-tenant `TenantDB` cannot select/insert/update/delete FLAT tables [42bbbc88]
...
- [x] Task: Write Red tests proving REFERENTIAL table misuse is detected behaviorally or by a non-vacuous static check [42bbbc88]
...
- [x] Task: Classify unregistered tables with explicit comments and audit-friendly reasons [42bbbc88]
...
- [x] Task: Update `createTenantDB` / context behavior to fail closed for null tenants [42bbbc88]
...
- [x] Task: Replace vacuous referential-scope detector with meaningful detection [42bbbc88]
...
- [x] Task: Run domain/db tests for tenant coverage and record results [42bbbc88]
...
- [x] Task: Write Red tests in `@reading-advantage/types` and auth/API contexts for active app roles including `INTERN`, `SALES_REP`, and `SALES_ADMIN` [12b7e88d]
...
- [x] Task: Align role schemas across `packages/types`, `packages/auth`, `packages/api`, and domain permission registration [12b7e88d]
...
- [x] Task: Write Red tests proving production rate limiting is not process-local and has per-user plus per-IP semantics [12b7e88d]
...
- [x] Task: Add auth-client response validation tests for malformed login/session payloads [12b7e88d]
...
- [x] Task: Run auth/types/API targeted tests [12b7e88d]
...
- [x] Task: Add Red tests for shared response envelopes: success, list, validation error, unauthorized, forbidden, not found, conflict, and internal error [102cb2c1]
...
- [x] Task: Add Zod contract tests to `@reading-advantage/types` for role, user/session, class, sales, and branded ID schemas [102cb2c1]
...
- [x] Task: Introduce or consolidate shared response/error contracts without breaking transport independence [102cb2c1]
...
- [x] Task: Replace at least one reviewed duplicated router-local contract with imported shared/domain contract as the adoption proof [102cb2c1]
...
- [x] Task: Replace string-based error mapping at the reviewed shared-foundation leakage site with typed errors [102cb2c1]
...
- [x] Task: Run types/API/domain tests [102cb2c1]
  ...
  - Green evidence (2026-06-28):
    ...
    - Full api suite at Phase 3 Green initially reported 201 tests pass / 2 failed in `auth-security-phase1-route-contracts.test.ts`. Correctness review later proved these were introduced by Wave 0 Phase 2 commit `076cda94` when `login.ts` started calling `configurePostgresRateLimiter(db)` at module load and the existing test mock was not updated. Fixed in review commit `4176c4c4` by adding `configurePostgresRateLimiter: vi.fn()` to the mock; current API suite passes 216/216.
...
- [x] Task: Write Red tests showing the reviewed `reports.teacherDashboard` query is delegated to a domain function, not implemented in the tRPC router [52a7351d]
...
- [x] Task: Move shared-foundation-identified business logic from API transport to domain functions [52a7351d]
...
- [x] Task: Add or update static guard tests forbidding Drizzle/schema imports in `packages/api/src/routers/**` except approved infrastructure exceptions [52a7351d]
...
- [x] Task: Verify router tests assert delegation and transport mapping only [52a7351d]
...
- [x] Task: Run API/domain lint, check-types, and tests [52a7351d]
  ...
  - Green evidence (2026-06-28):
    ...
    - Full API suite at Phase 4 Green initially reported 214/216 pass. Correctness review later proved the 2 `auth-security-phase1-route-contracts.test.ts` failures were introduced by Wave 0 Phase 2 commit `076cda94`, not pre-existing. Review commit `4176c4c4` updated the auth mock with `configurePostgresRateLimiter: vi.fn()`; current API suite passes 216/216. This plan note was corrected to avoid A5 false-claim text.
...
- [x] Task: Run all required verification commands from `spec.md` [30d2749a]
...
- [x] Task: Update `measure/audit-reports/monorepo-review-roadmap_20260626/critical-high-remediation-plan.md` with a Wave 0 completion note only if all AC pass [50569fb0]
...
- [x] Task: Add lessons learned for tenant fixtures, contract tests, and rate-limit production safety if new patterns are discovered [50569fb0]
...
- [x] Task: Mark resolved tech-debt rows only when behavior tests prove the fixes [50569fb0]
...
- [x] Task: Run Measure phase acceptance and archive the track [50569fb0]
