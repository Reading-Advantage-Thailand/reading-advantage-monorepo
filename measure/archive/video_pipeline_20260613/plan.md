...
- [b] Task: Run `CI=true pnpm --filter marketing build` — green.
  - deferred:repo-owner — build is blocked by a vinext ↔ vite environment-incompatibility class that touches infrastructure decisions (dependency upgrades, lockfile reshuffles, possibly peer-dep fan-out across the workspace) and is out of scope for AI-agent Jr-Green work.
  - **Diagnosis (2026-06-30, Jr-Green investigation, this commit).** `vinext@0.1.8` imports `{ parseSync } from "vite"` in `node_modules/vinext/dist/build/report.js` (line 3). That import only resolves against `vite@8.x` — the monorepo currently pins `apps/marketing` to `vite@^6.0.0` so the ESM loader fails synchronously with `SyntaxError: The requested module 'vite' does not provide an export named 'parseSync'`. Tried three candidate fixes; none is safe to land:
    1. **`vite: ^6 → ^7`** — `vite@7.3.6` (latest of v7) exports only `parseAst` / `parseAstAsync`, **not** `parseSync`. Same `parseSync` failure. Reverted.
    2. **`vite: ^6 → ^8` + add `@vitejs/plugin-rsc`** — `vite@8.1.0` does export `parseSync`, satisfying the original error. But the build now progresses to `[1/5] analyze client references` and the `builder.build(builder.environments.rsc)` call (from `@vitejs/plugin-rsc@0.5.27`) fails with `[UNLOADABLE_DEPENDENCY] Could not load .../shims/{headers,server,font/google,font/local,navigation}` for every `next/*` import the RSC virtual entry pulls in. The error is the same class already documented in the Phase 1 plan ("upgrading to vite@8 exposes a vinext framework bug (RSC entry's hardcoded next/font/google import unresolved in Vite 8 RunnableDevEnvironment.fetchModule) — tracked as infrastructure limitation, not Phase 1 defect"). Reverted.
    3. **Pin vinext to an older 0.1.x** — every released vinext from 0.1.0 through 0.1.8 imports `parseSync` from `vite` with the identical Babel-parser-style signature (`astType: "ts"`, `sourceType: "module"`, `result.errors[].severity`). No version is parseSync-free. Reverted.
  - **Why this is not a Jr-Green scope fix.** All three candidates are dependency upgrades or downgrades on a tooling-pinned build path that the Phase 1 plan already flagged as a known infrastructure limitation, with documented risk of framework-bug exposure in Vite 8. Per the work-distribution guidance ("if the fix requires a broad dependency upgrade with high blast radius, do not perform it"), this is deferred to the repo-owner (Phikul / Daniel).
  - **Recommended remediation options for repo-owner.** (a) File an upstream issue against `vinxi/vinext` asking the maintainers to either vendor `@babel/parser` directly (the Babel-shape `parseSync` is a legacy Vite re-export that may be removed again) or fix the `next/*` aliases in the RSC environment so extension-suffix resolution works. (b) Add a `pnpm.overrides` entry that pins `vinext` to a yet-to-be-released or patched version once upstream lands a fix. (c) Until either lands, keep the `vinext build` gate out of the marketing app's required-CI set (mirroring the existing dev-server-gate-skip noted in Phase 1 plan line 16) and document the accepted aggregate-suite exception.
  - **Reproducer.** `cd /home/daniel-bo/Desktop/reading-advantage-monorepo && CI=true pnpm --filter marketing build` exits 1 with `SyntaxError: The requested module 'vite' does not provide an export named 'parseSync'` at module-load time, before any user code runs.
