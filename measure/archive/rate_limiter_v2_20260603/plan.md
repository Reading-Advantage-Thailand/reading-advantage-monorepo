...
- [x] Task: Confirm `packages/db` migrations apply cleanly against `science_advantage_test`. *(confirmed @ 076cda94 — migration `0024_futuristic_vulture.sql` applied; auth test suite runs without schema errors)*
- [x] Task: Read `packages/auth/src/rate-limit.ts` and `packages/api/src/routes/auth/login.ts:36-132`; understand the current surface. *(done @ 076cda94 — see test-strategy §0 baseline table)*
- [x] Task: Coordinate with Track 4 (Audit Log) — failed-login events are audit-worthy; this track emits them as a side effect. *(de facto satisfied @ 076cda94 — `login.ts:155-168` already emits `auth:login_failed` / `auth:login`; verify the captcha change preserves the emit)*
...
- [x] Task: Add `loginAttempts` table to `packages/db/src/schema/auth.ts` (FR-1). *(shipped at baseline @ 076cda94)*
- [x] Task: Add to `packages/db/src/schema/index.ts` barrel. *(shipped @ 076cda94 — `export * from "./auth.js"`)*
- [x] Task: Generate Drizzle migration: `pnpm --filter @reading-advantage/db drizzle-kit generate`. Inspect the generated SQL. *(shipped @ 076cda94 — `0024_futuristic_vulture.sql`)*
- [x] Task: Apply migration to `science_advantage_test`. Verify table + indexes. *(applied @ 076cda94; auth test suite runs clean)*
...
- [x] Task: Create `packages/auth/src/rate-limit-store.ts` with the 3 functions (FR-3). *(shipped @ 076cda94 — `createPostgresRateLimitStore` implements get/set/delete)*
- [x] Task: Implement with `SELECT ... FOR UPDATE` upsert pattern. *(shipped @ 076cda94 — `.for("update")` + `onConflictDoUpdate`)*
- [x] Task: Write failing tests: *(shipped @ 076cda94 + passing — these are regression guards, not TDD-Red, since the impl already exists; see test-strategy §0)*
...
- [x] Task: Implement. Confirm tests pass. *(confirmed @ 076cda94 — 17 rate-limit tests green)*
...
- [x] Task: Replace the in-memory `Map` with calls to the new Postgres-backed store. *(shipped @ 076cda94 — `configurePostgresRateLimiter` is the production default)*
- [x] Task: Keep the same public API (`checkRateLimit`, `recordFailure`, `resetLimit`) for backward compatibility. *(shipped @ 076cda94 — signatures are `checkRateLimit(username, ip?)`, etc.)*
- [x] Task: **Dev-only fast-path**: if `NODE_ENV === 'development'` AND `RATE_LIMIT_INMEMORY_FASTPATH === 'true'`, use the in-memory `Map`. Default: Postgres-backed. *(shipped @ 076cda94 — `isInMemoryFastPathEnabled()` dual gate)*
- [x] Task: Add per-IP rate limit: `checkRateLimitByIp(ip, config)` that calls the same store with `kind: 'ip'`. *(shipped differently @ 076cda94 — IP is folded into `checkRateLimit(username, ip?)`; see CR-1. A standalone `checkRateLimitByIp` export does NOT exist — resolve in Phase 6.)*
- [x] Task: Write failing test: `checkRateLimit` with a Postgres backend uses the new store; with the dev fast-path flag set, uses the in-memory `Map`. *(shipped @ 076cda94 — `rate-limit.test.ts` covers the in-memory path; the dual-gate fast-path assertion is a gap — see test-strategy §2 Phase 3 closeout gate)*
- [x] Task: Confirm tests pass; existing `packages/auth` test suite still passes. *(confirmed @ 076cda94 — 8/8 rate-limit tests green)*
...
- [x] Task: Confirm. *(Green @ abfd4505 + 3e48f386 — 12/12 cleanup tests pass; batching, advisory lock, and idempotent job lifecycle verified.)*
...
- [x] Task: In `packages/api/src/routes/auth/login.ts:36-132`: *(wiring shipped at baseline @ 076cda94)*
...
- [b] Task: Write failing integration tests:
  ...
  - **Red-phase evidence (2026-07-03):**
    ...
    - Integration test `apps/science-advantage/src/__tests__/rate-limit-login.integration.test.ts` authored.
      ...
      is restored.
- [x] Task: Confirm. *(Green @ abfd4505 — `checkRateLimit` returns `captchaRequired` after the username bucket reaches the threshold; the login 401/429 responses include `captchaRequired: true`; `resetLimit` clears the captcha state. Unit suite is green; integration suite is authored and gated on infra.)*

## Phase 6: Update `packages/auth` Exports

- [x] Task: Re-export `checkRateLimitByIp`, `RateLimitConfig`, `RateLimitCheckResult` from `packages/auth/src/index.ts`. *(Green @ review 3e48f386 — `checkRateLimitByIp` wrapper added (delegates to `checkIdentifier` with `kind: 'ip'`), `RateLimitConfig`, `RateLimitCheckResult`, `DEFAULT_IP_RATE_LIMIT_CONFIG` all exported. CR-1 resolved: IP-only check is now available via `checkRateLimitByIp(ip, config?)`.)*
...
- [x] Task: Update `packages/auth/src/rate-limit.ts` JSDoc to mark the in-memory `Map` as "dev-only fast-path; production uses Postgres-backed store." *(shipped @ 076cda94 — `rate-limit.ts:79-86`)*
...
- [x] Task: Document the cross-app impact in a lessons-learned entry. *(captured for Phase 9 @ 12ee1d94)*
...
- [b] Task: Update `measure/tech-debt.md` row `audit_20260603_housekeeping_batch` to mark F-403, F-407 `Resolved`. *(deferred:infra — A6 defense: only after Phase 5 integration tests are green (INFRA-2); "resolved" requires the live-DB adversarial test to pass. Unit + adversarial suites are green but the DB-backed integration proof awaits drizzle-kit migrate global-setup restoration.)*
- [x] Task: Add a lessons-learned entry: "In-memory `Map` is the wrong default for security state; Postgres-backed rate limiting is one roundtrip more per login, but correctness across replicas is non-negotiable. Trusting X-Forwarded-For without TRUST_PROXY_COUNT enables IP spoofing and bucket poisoning; per-identifier atomic increments prevent under-counting under concurrency. The captcha trigger is a hook only — flag emission without verification does not by itself stop bots, but it gives the frontend a contract to render a challenge once a threshold is crossed." *(Green 2026-07-03 @ 12ee1d94 — entry added to measure/lessons-learned.md)*
- [x] Task: Add a follow-up track placeholder in `measure/tracks.md` under Pending Tracks: "Captcha Verification (reCAPTCHA/hCaptcha/Turnstile integration)." *(Green 2026-07-03 @ 12ee1d94 — placeholder added to measure/tracks.md)*
- [x] Task: Move track to `measure/archive/rate_limiter_v2_20260603/` and update `measure/tracks.md`. *(Green 2026-07-03 @ 12ee1d94 — dir moved, tracks.md updated, A13/A9 defenses verified)*
