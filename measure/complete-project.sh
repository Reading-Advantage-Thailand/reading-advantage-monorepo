...
# Discovers phases with unfinished tasks ([ ] or [~]) and runs three steps
# per phase:
#   1. Implementation  — opencode / qwen3.6-plus-free
#   2. Review          — kimi CLI
#   3. Final review    — opencode / glm-5.1  (also clears codecamp tech-debt items)
#
# Fully-completed phases (all tasks [x]) are automatically skipped.
#
# Usage:
...
# ── Codecamp tracks in execution order ────────────────────────────────────
CODECAMP_TRACKS=(
  "codecamp_advantage_20260513"
  "codecamp_curriculum_20260514"
  "codecamp_review_remediation_20260515"
)
...
tracks = ['codecamp_advantage_20260513','codecamp_curriculum_20260514','codecamp_review_remediation_20260515']
repo = '$REPO_ROOT'
...
for tid in tracks:
    ...
    # Split text into phase-level blocks
    ...
    for phase in phases:
        ...
        # Count incomplete task lines ([ ] or [~])
        incomplete = len(re.findall(r'^\s*-\s+\[[ ~]\]', phase, re.MULTILINE))
        total = len(re.findall(r'^\s*-\s+\[', phase, re.MULTILINE))
        if incomplete > 0:
            # Clean heading for display: strip ## prefix and [checkpoint:]/[final-verification:] tags
