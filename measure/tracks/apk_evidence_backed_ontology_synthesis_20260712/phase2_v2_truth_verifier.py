#!/usr/bin/env python3
"""Verifies the corrected Phase 2 capability-ontology truth contract."""
...
FIXTURE_MANIFEST = "phase2-v2-fixture-manifest.json"
MAX_BATCH_GAMES = 3
...
def _truth_contract(track_root: Path) -> dict[str, str]:
    """Returns the published v2 truth-package bindings required in receipts."""
    paths = (
...
def _verify_inputs(
    ...
    if (
        ...
        or dispatch.get("schema_version") != "apk-t9-phase2-role-dispatch.v2"
        or acceptance.get("decision") != "ACCEPT_PHASE1_OPEN_PHASE2"
    ):
        _add(findings, "PHASE2_INPUT_DRIFT", "The v2 authority gate is not active.")
    if (
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = {
        "schema_version": "apk-t9-phase2-v2-verification-result.v1",
        "state": result.state,
