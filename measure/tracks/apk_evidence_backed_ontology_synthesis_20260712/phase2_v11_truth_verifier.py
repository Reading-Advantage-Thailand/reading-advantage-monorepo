#!/usr/bin/env python3
"""Verifies Phase 2 v11 taxonomy-backed record dispositions and authority."""
...
DISPATCH_PATH = "phase2-role-dispatch-v11.json"
DISPATCH_SHA256 = "5b4b87ef31382f29d61499804d57a82bcb4d6baa37fa4a98e6438c2c365c63e5"
FIXTURE_MANIFEST = "phase2-v11-fixture-manifest.json"
ROOT_SEAL = "phase2-v11-root-truth-seal.json"
MAPPER_RELEASE = "phase2-v11-mapper-release.json"
REVIEW_OUTPUT = "phase2-v11-independent-review.json"
REVIEW_RECEIPT = "role-receipts/phase2/capability-reviewer-v11.json"
...
BASE_TRUTH_PATHS = (
    DISPATCH_PATH,
    "phase2_v11_truth_verifier.py",
    "phase2_v11_truth_verifier_test.py",
    FIXTURE_MANIFEST,
    "phase2-v11-red-report.json",
    "role-receipts/phase2/truth-test-author-v11.json",
)
PRESERVED_V10 = {
    "phase2-role-dispatch-v10.json": "b7dc7be6ec593623a21656afd3215b2764188a41915e0b2e709be6e338e26eb7",
    "phase2_v10_truth_verifier.py": "3cf4eb598055a8d03e5ea5a38bef79f5bf2ec1b50c43b48d142ef26c5b70d1e8",
    "phase2_v10_truth_verifier_test.py": "920209f26b3cd13a9e7545531a36c80ff21ee0e5ea4be861958e312ccfc3abb2",
    "phase2-v10-fixture-manifest.json": "99be6b366188670d9e73ff10f684a695fb08c6b31552714c2edeb33cc9cfdd5e",
    "negative-fixtures/phase2-v10/end-to-end-attacks.json": "cbf5d09d9b84faf95a2437f00a4f0c8baf304193b8a1e1838cb4f09ada2dc772",
    "phase2-v10-red-report.json": "67b56ef72ffbd5bc9a5503dfaa4d5dc660978baccd93e0271969b0c10653ef4a",
    "role-receipts/phase2/truth-test-author-v10.json": "94c1b9c53aca2857f46da9ec6f7290d609fa001703dd3881009f3c6aa9818149",
    "phase2-v10-root-truth-seal.json": "5f5c0677b8ceea2b2f1751ff651f69c3ef6218abb0cc4a49e8779088bd5db8af",
    "phase2-v10-mapper-release.json": "8eefc6703efab3fe4ace996eb847fb4f5feade7b6f78663c44fe27de9694f6b0",
}
AUDIT_KEYS = {
...
class Finding:
    """Represents one deterministic v11 finding."""
...
class VerificationResult:
    """Represents the public v11 verification result."""
...
def _verify_inputs(
    ...
    """Verifies v11 authority, all v10 bytes, and inherited accepted inputs."""
    fixed = {DISPATCH_PATH: DISPATCH_SHA256, **PRESERVED_V10}
    ...
    if (
        dispatch.get("schema_version") != "apk-t9-phase2-role-dispatch.v11"
        or dispatch.get("status") != "active-context-disposition-truth-repair"
    ):
        _add(findings, "TRUTH_INPUT_DRIFT", "The v11 dispatch is not active.")
    prior: list[v10.Finding] = []
    inputs, registry, checks = v10._verify_inputs(track_root, prior)
    if prior:
        _add(findings, "TRUTH_INPUT_DRIFT", "Inherited v10 authority differs.")
...
def _validate_manifest(
    ...
    """Validates exact v11 manifest and declared fixture identities and counts."""
    ...
    if (
        set(manifest) != top
        or manifest.get("schema_version") != "apk-t9-phase2-v11-fixture-manifest.v1"
    ...
    for row in fixtures:
        ...
        if (
            ...
            or fixture.get("schema_version") != "apk-t9-phase2-v11-end-to-end-attacks.v1"
...
def _verify_root_authority(
    ...
    """Validates v11 root authority before mapper presence."""
    seal_path = track_root / ROOT_SEAL
    release_path = track_root / MAPPER_RELEASE
    if not seal_path.is_file() or not release_path.is_file():
        _add(findings, "ROOT_V11_AUTHORITY_MISSING", "Root v11 authority is not published.")
        return None, None, "RED_WAITING_FOR_ROOT_V11_AUTHORITY"
    if expected_release is None:
        _add(findings, "EXPECTED_MAPPER_RELEASE_REQUIRED", "Expected v11 release SHA-256 is required.")
        return None, None, "INVALID"
    if v9._sha(release_path) != expected_release:
        _add(findings, "MAPPER_RELEASE_MISMATCH", "Expected v11 release SHA-256 differs.")
        return None, None, "INVALID"
    ...
    if (
        ...
        or seal.get("schema_version") != "apk-t9-phase2-root-truth-seal.v11"
        or seal.get("track_id") != TRACK_ID
        or seal.get("dispatch_sha256") != DISPATCH_SHA256
        or seal.get("status") != "sealed-red-v11"
        or seal.get("pins") != live
        or set(live) != set(truth_paths)
    ):
        _add(findings, "LIVE_TRUTH_DRIFT", "Root v11 seal differs from live truth.")
    ...
    if (
        ...
        or release.get("schema_version") != "apk-t9-phase2-mapper-release.v11"
        ...
        _add(findings, "MAPPER_RELEASE_MISMATCH", "Root v11 release differs.")
...
def _validate_mapper_receipt(
    ...
    """Validates exact v11 mapper identity and six output hashes."""
    ...
    if (
        ...
        or receipt.get("task_id") != "phase2-curated-evidence-mapper-v5-v11"
        ...
        _add(findings, "STALE_OR_WRONG_MAPPER_RECEIPT", "Mapper v11 receipt differs.")
...
def _validate_review(
    ...
    """Validates v11 taxonomy and expanded record verdicts plus inherited review."""
    ...
    if (
        ...
        or review.get("schema_version") != "apk-t9-phase2-independent-review.v11"
    ...
    for row in record_reviews if isinstance(record_reviews, list) else []:
        ...
        if (
            ...
            rejected = True
    # Project expanded v11 review to the sealed v9 semantic reviewer.
    ...
    if (
        ...
        or receipt.get("task_id") != "phase2-curated-evidence-review-v11"
        or receipt.get("dispatch_sha256") != DISPATCH_SHA256
        or receipt.get("root_truth_seal_sha256") != seal_sha
        or receipt.get("root_mapper_release_sha256") != release_sha
        ...
        _add(findings, "STALE_OR_WRONG_REVIEWER_RECEIPT", "Reviewer v11 receipt differs.")
    ...
    if rejected:
        _add(findings, "SEMANTIC_REVIEW_REJECTED", "Independent v11 review rejected the candidate.")
    return rejected
...
def verify_phase2(
    ...
    """Runs public file-backed v11 verification in authority-first order."""
    del repo_root
    findings: list[Finding] = []
    try:
        ...
        _add(findings, "INVALID_SCHEMA", "The v11 file-backed candidate cannot be validated.")
        return VerificationResult("INVALID", tuple(sorted(findings, key=lambda row: row.code)), 0)


def main(argv: list[str] | None = None) -> int:
    """Runs v11 verification and emits stable JSON."""
    ...
    print(json.dumps({
        "schema_version": "apk-t9-phase2-v11-verification-result.v1",
        "state": result.state,
