"""Focused tests for the corrected and sealed Phase 2 v2 truth contract."""
...
import phase2_v2_truth_verifier as verifier

TRACK_ROOT = Path(__file__).resolve().parent
REPO_ROOT = TRACK_ROOT.parents[2]


class Phase2V2TruthVerifierTest(unittest.TestCase):
    """Exercises sealed Red, the valid baseline, and every v2 counterexample."""

    def test_live_state_is_red_only_for_missing_mapper_outputs(self) -> None:
        """Absent mapper outputs must be the sole live v2 Red finding."""
    ...
    def test_fixture_manifest_is_exact_and_bounded(self) -> None:
        """All grouped fixture files must match immutable v2 bindings."""
    ...
    def test_all_bound_negative_fixtures_fail_exactly_under_budget(self) -> None:
        """Every grouped v2 counterexample must emit exactly its sealed codes."""
        manifest = json.loads((TRACK_ROOT / verifier.FIXTURE_MANIFEST).read_text())
    ...
    def test_cli_accepts_only_exact_missing_output_red(self) -> None:
        """Expected-code mode binds the sealed live v2 Red exactly."""
        args = [
            "--repo-root",
            str(REPO_ROOT),
            "--track-root",
