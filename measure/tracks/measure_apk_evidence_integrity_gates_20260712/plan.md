...
- [x] Task: Freeze attempts 1–5 as immutable, named negative fixtures; record the baseline gate commit, allowed-input hashes, candidate-output paths, and gate-edit prohibition. (Green: contracts.validate_fixture_manifest enforces every attempt is represented with a stable rejection code and paired valid control; the manifest freezes baseline_gate_commit f61eb643 + gate_edit_prohibition. SHA: pending — recorded below after the Green commit lands.)
- [x] Task: Define versioned envelope, numeric-budget/unit, severity, stop-loss, acceptance, revocation, and canonical `depends_on` contracts; reject `unmeasured` rather than defaulting it. (Green: parse_labeled_budget rejects `unmeasured` + generic-quantity nouns, validate_envelope rejects accepted/missing-baseline/unknown-schema, validate_dependency_field rejects the `dependencies` alias, validate_plan_marker rejects legacy `[ ]`. SHA: pending.)
- [x] Task: Build the fixture harness and its positive-control corpus without encoding APK product conclusions. (Green: test_contract_scaffold passes 34 tests exercising A3/A4/A5/A6/A8/A12 defenses with no APK product conclusions baked into assertions. The A12 test was revised truthfully to record concrete repo-wide debt rather than over-claiming catalog integrity. SHA: pending.)
