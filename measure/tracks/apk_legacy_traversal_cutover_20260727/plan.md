...
- The focused binding contract uses `/28` for `SW-INPUT-002`. The remaining failure is the unchanged `spellweavers-run.json` pointer `/20`; the archive resolves `/20` to `SW-TRANS-003`, and `/19` is recommended.
