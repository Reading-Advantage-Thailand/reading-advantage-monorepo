...
Green base: `466ec180a`
Dossier commit: `a0e27d909`
...
The immutable Red test retains one unresolved pointer mismatch outside the final `SW-ASSET-001` correction.
...
- Spellweaver's Run still declares `/19` as `SW-COLL-001`; the archive resolves `/19` to `SW-TRANS-002`.
- The recommended archive pointer for `SW-COLL-001` is `/18`.

The dossier data cannot satisfy both the immutable `/19` Red expectation and the archive claim identity.
...
Review A must return the remaining `SW-COLL-001` mismatch to Mid Red.
...
commits: a0e27d909; evidence commit pending
...
limitations: immutable Red pointer drift remains for Spellweaver's Run `SW-COLL-001`
