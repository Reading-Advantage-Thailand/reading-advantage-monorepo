...
- [x] Commit the 241 OGG files, `IMPORT-RECEIPT.tsv`, `standard-pack-release.json`,
...
- [x] Evidence: `node scripts/verify-standard-pack-parity.mjs` output captured
      into the commit body or a track receipt.

## Phase 2: Red — pin the new release identity in tests

- [x] Update the six test files with old-release fixtures/literals to expect
...
- [x] Commit as `test(apk): ... (track_id: ...)`.

## Phase 3: Green — advance the constants

- [x] `accepted-standard-pack-release.ts`: version, three digests, assetCount,
      acceptedAt `2026-08-04`.
- [x] `capability-manifest.ts` (literal type + constants), `scaffolding/exemplar.ts`,
...
- [x] Scoped vitest run (13 files enumerated in session log) green.
- [x] Commit as `feat(apk): accept standard-pack release 2026.08.04 (audio cohort) (track_id: ...)`.

## Phase 4: Suitability evidence pins + closeout

- [x] `existing-core-suitability.ts` CATALOG_SHA256 + IMPORT_RECEIPT_SHA256;
      ...
      literals blindly.
- [x] Their scoped tests green; parity script output attached to a closeout
      receipt `release-acceptance-receipt-20260804.md` in this track dir.
- [x] Commit `feat(apk): advance suitability evidence pins to release 2026.08.04 (track_id: ...)`.
- [x] Register track in `measure/tracks.md`; close with status complete.
