...
- [x] Task: Design the vinext production Dockerfile
  ...
  - [x] Multi-stage: deps (`pnpm install --frozen-lockfile`) → dependency-aware builder (`pnpm turbo run build --filter=marketing`) → runner that ships `dist/` + minimal runtime and runs `vinext start` on `PORT=8080`, `HOSTNAME=0.0.0.0`
