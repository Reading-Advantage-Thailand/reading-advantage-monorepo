...
The proxy may make only an optimistic redirect decision from cookie presence;
it must not treat a cookie or client-provided role as a valid session or
...
- Preserve the current lower-case Primary role policy (`student`, `teacher`,
  `admin`, `system`) and authenticated settings access.
...
- Do not weaken server authorization, trust client claims, add shims, use
  `ignoreBuildErrors`, or suppress failing tests.
...
4. Missing, malformed, expired, and revoked session cookies cannot pass a
   protected server-side route/API guard; valid users retain only their
   existing allowed route groups.
