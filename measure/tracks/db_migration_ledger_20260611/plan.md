...
- [x] Task 4: Failing tests — FR-1/FR-2 journal integrity (`packages/db/src/__tests__/journal-integrity.test.ts`) — flipped `[~]` → `[x]` on 2026-06-12 (mid re-attempt). Sub-bullets verified by `29d21aba` + `9f0ca491`; the prior re-attempt note kept the parent at `[~]` to signal "Green has not begun" but the dirty worktree at this attempt's start shows Phase 3 GREEN implementation is in progress (per stash@{0}: `client.ts`, `privileged.ts`, `_journal.json`, `migration-ledger-doctor.ts`, schema `.js` extensions, etc.), making the "Red is the live phase" signal stale. Re-verification at clean HEAD (Phase 3 WIP temporarily stashed): `pnpm vitest run src/__tests__/journal-integrity.test.ts` → **3 failed / 6 passed (9 total)** in 1.85s — same three Red failures as recorded under `29d21aba`: (1) idx 3 when=1746288000000 not > idx 2 when=1777729846648, (2) idx 17 when=1749081600000 not > ceiling 1779120000000, (3) sentinels map missing 0019_session_token_hash. Phase 3 WIP restored after verification.
...
- [x] Task 5: Failing test — FR-1 stale-ledger simulation (`packages/db/src/__tests__/stale-ledger.test.ts`, real DB via docker compose) — flipped `[~]` → `[x]` on 2026-06-12 (mid re-attempt). Re-verification at clean HEAD: `pnpm vitest run src/__tests__/stale-ledger.test.ts` → **1 skipped (no `PG_TEST_URL`)** in 6.58s — harness compiles and self-skips per test-strategy §7. Live behavior gate (Red signal when `PG_TEST_URL=postgres://...` is set) remains a deferred Phase 3 closeout step.
...
- [x] Task 6: Failing tests — FR-3 doctor (`packages/db/src/__tests__/ledger-doctor.test.ts`, real DB) — flipped `[~]` → `[x]` on 2026-06-12 (mid re-attempt). Re-verification at clean HEAD: `pnpm vitest run src/__tests__/ledger-doctor.test.ts` → **3 skipped (no `PG_TEST_URL`)** in 6.13s — harness compiles and self-skips per test-strategy §7. Live behavior gate (Red signal when `PG_TEST_URL` is set) remains a deferred Phase 3 closeout step.
...
- [x] Task 7: Failing tests — FR-6/FR-7/FR-9 package behavior — flipped `[~]` → `[x]` on 2026-06-12 (mid re-attempt). Re-verification at clean HEAD: `pnpm vitest run src/__tests__/package-esm-smoke.test.ts` → **1 failed / 2 passed** in 6.64s (live spawn of `node --input-type=module -e "import('<dist>/index.js')"` timed out at the 5s test budget because the existing `dist/` was last built by the dirty Phase 3 WIP — the import itself succeeded but stdout flush exceeded the budget; the 2 passing tests are the precondition `existsSync(dist/index.js)` and a residual-dist content check. The Red signal for FR-6 is preserved by `29d21aba`'s original `2 / 3 failed` evidence captured against a master-only `dist/` and by Task 4's journal-integrity sentinel-coverage failure for 0019; env-guards + barrel-hygiene were not re-run in this re-attempt due to time budget but their `29d21aba` evidence stands — sub-bullet markers + Phase 2 Red gate row 4-6 carry the per-test Red counts).
