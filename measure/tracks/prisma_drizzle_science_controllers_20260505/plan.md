...
- [~] Task: Verify clean install + build + test — **BLOCKED**: clean `pnpm install --frozen-lockfile` (24s, no lockfile changes) + tsc (360 errors, 0 NEW, exact baseline match) + lint (4 errors, 6 warnings, 0 NEW, exact baseline match) + test (213 failed / 747 passed; ALL pre-existing buckets: testing-library matcher narrowing, lib/auth/{password,rate-limit} missing siblings, mastery-profile integration `science_standards_framework_code_unique` state-leak, e2e playwright misrouted to vitest; 0 Prisma references). **Build FAILS with NEW Prisma-removal regression**: Turbopack 4 errors `Module not found: Can't resolve 'fs'/'net'/'tls'/'perf_hooks'` — `postgres@3.4.9` (node-only) bleeds into Client Component bundles via `lib/gamification/badges.ts` → `@reading-advantage/db`, imported by client `components/features/student/quiz-player.tsx` (`BADGE_DEFINITIONS`) and `app/(student)/student/classes/[classId]/lessons/[lessonSlug]/page.tsx`. Server-side import traces also fail (postgres needs `serverExternalPackages` or webpack/turbopack externals). Pre-Prisma `badges.ts` imported `@/lib/prisma` which Next bundled cleanly; migration commit `1fea05c` (plan row 233, "migrate lib/gamification/badges.ts off Prisma") introduced the leak. Needs dedicated follow-up: either (a) split `badges.ts` into `badges.constants.ts` (client-safe BADGE_DEFINITIONS) + `badges.server.ts` (DB checkers), or (b) configure `next.config.ts` `serverExternalPackages: ['postgres', '@reading-advantage/db']` and audit all client imports. Capture: install 24s; tsc 80s; lint 75s; build 52s (FAIL); test 130s.
    ...
    - [ ] Sub-task: Fix xp.ts client-bundle leak — split into `xp.constants.ts` (client-safe LEVEL_THRESHOLDS/LEVEL_NAMES + pure fns `calculateLevel`/`getLevelName`/`calculateXpForQuiz`) and `xp.ts` (server-only shim with `import "server-only"`; keeps `awardXp` DB writer + re-exports constants); repoint client importer (level-up-animation)
