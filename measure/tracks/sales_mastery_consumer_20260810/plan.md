...
- [x] Task: Implement the tenant mapping and durable idempotent projection port
  ...
  log.
- [x] Task: Remediate Review A with a corrected registry path and disposable
  PostgreSQL 16 behavior tests. Safe Red exposed the migration-number collision
  and snapshot-chain failure. Live Red exposed the malformed legacy migration and
