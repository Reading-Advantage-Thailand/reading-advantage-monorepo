# Shadow Gate Dungeon Mechanic Blueprint

## Identity and fantasy

Escape a shadow creature with ordered crystals. Canonical ID: `game:shadow-gate-dungeon`; confidence: **high**.

## Learning loop

The player must collect sentence crystals in order while remaining outside detection. Correct input advances one content step; incorrect or out-of-order input records feedback and an attempt without silently skipping required content.

## World and controls

free-roam dungeon with stealth radius and indicators. The Phaser rebuild may choose native physics, cameras, pooling, and scene composition while preserving recognizable agency and target readability.

## Progression and terminal state

complete the sentence and escape or lose health/time to the pursuer. Score, accuracy, correct answers, and attempts map to the established `GameResults`; authoritative XP and persistence remain host-owned.
