# Archer's Revenge Mechanic Blueprint

## Identity and fantasy

Become a precision archer. Canonical ID: `game:archers-revenge`; confidence: **medium**.

## Learning loop

The player must shoot enemies matching the target translation and spare shielded decoys. Correct input advances one content step; incorrect or out-of-order input records feedback and an attempt without silently skipping required content.

## World and controls

pointer/touch aiming and projectile combat. The Phaser rebuild may choose native physics, cameras, pooling, and scene composition while preserving recognizable agency and target readability.

## Progression and terminal state

clear target waves; incorrect or shielded hits consume attempts. Score, accuracy, correct answers, and attempts map to the established `GameResults`; authoritative XP and persistence remain host-owned.
