# Rune Match Mechanic Blueprint

## Identity and fantasy

Defeat monsters by matching language runes. Canonical ID: `game:rune-match`; confidence: **high**.

## Learning loop

The player must select matching term and translation runes. Correct input advances one content step; incorrect or out-of-order input records feedback and an attempt without silently skipping required content.

## World and controls

pointer/touch puzzle board with combat feedback. The Phaser rebuild may choose native physics, cameras, pooling, and scene composition while preserving recognizable agency and target readability.

## Progression and terminal state

clear the encounter set; mistakes consume attempts or enemy turns. Score, accuracy, correct answers, and attempts map to the established `GameResults`; authoritative XP and persistence remain host-owned.
