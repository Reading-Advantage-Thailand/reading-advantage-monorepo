...
Restore an astral ritual by shooting crystals. Canonical ID: `game:astral-mage`; confidence: **medium**.

## Learning loop

The player must shoot word crystals in sentence order. Correct input advances one content step; incorrect or out-of-order input records feedback and an attempt without silently skipping required content.

## World and controls

free movement plus directional aim/fire in a bounded void. The Phaser rebuild may choose native physics, cameras, pooling, and scene composition while preserving recognizable agency and target readability.

## Progression and terminal state

complete the ritual sequence or lose health/time. Score, accuracy, correct answers, and attempts map to the established `GameResults`; authoritative XP and persistence remain host-owned.
