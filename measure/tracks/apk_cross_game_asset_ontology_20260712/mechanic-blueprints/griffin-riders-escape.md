# Griffin Rider's Escape Mechanic Blueprint

## Identity and fantasy

Escape through sentence gates on a griffin. Canonical ID: `game:griffin-riders-escape`; confidence: **medium**.

## Learning loop

The player must fly through ordered word gates while avoiding obstacles. Correct input advances one content step; incorrect or out-of-order input records feedback and an attempt without silently skipping required content.

## World and controls

vertical/lane steering in a scrolling sky. The Phaser rebuild may choose native physics, cameras, pooling, and scene composition while preserving recognizable agency and target readability.

## Progression and terminal state

complete the sentence flight or lose collision health. Score, accuracy, correct answers, and attempts map to the established `GameResults`; authoritative XP and persistence remain host-owned.
