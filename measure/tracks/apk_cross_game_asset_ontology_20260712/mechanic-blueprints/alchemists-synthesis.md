# Alchemist's Synthesis Mechanic Blueprint

## Identity and fantasy

Synthesize spells in an alchemy workshop. Canonical ID: `game:alchemists-synthesis`; confidence: **high**.

## Learning loop

The player must match and merge vocabulary ingredients into valid recipes. Correct input advances one content step; incorrect or out-of-order input records feedback and an attempt without silently skipping required content.

## World and controls

drag/tap workstation with deterministic board state. The Phaser rebuild may choose native physics, cameras, pooling, and scene composition while preserving recognizable agency and target readability.

## Progression and terminal state

complete recipes before attempts or time expire. Score, accuracy, correct answers, and attempts map to the established `GameResults`; authoritative XP and persistence remain host-owned.
