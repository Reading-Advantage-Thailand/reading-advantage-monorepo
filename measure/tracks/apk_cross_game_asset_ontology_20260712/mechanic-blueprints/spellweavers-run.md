# Spellweaver's Run Mechanic Blueprint

## Identity and fantasy

Weave a sentence while sprinting through a forest. Canonical ID: `game:spellweavers-run`; confidence: **medium**.

## Learning loop

The player must change lanes to collect word orbs in order. Correct input advances one content step; incorrect or out-of-order input records feedback and an attempt without silently skipping required content.

## World and controls

three-lane runner using keys or lane taps. The Phaser rebuild may choose native physics, cameras, pooling, and scene composition while preserving recognizable agency and target readability.

## Progression and terminal state

finish the sentence/course; wrong orbs break the sequence or cost health. Score, accuracy, correct answers, and attempts map to the established `GameResults`; authoritative XP and persistence remain host-owned.
