# Devourer Slime Mechanic Blueprint

## Identity and fantasy

Grow from tiny slime to knight-devourer. Canonical ID: `game:devourer-slime`; confidence: **high**.

## Learning loop

The player must eat words in sentence order to unlock growth tiers. Correct input advances one content step; incorrect or out-of-order input records feedback and an attempt without silently skipping required content.

## World and controls

directional forest-arena movement and tiered collisions. The Phaser rebuild may choose native physics, cameras, pooling, and scene composition while preserving recognizable agency and target readability.

## Progression and terminal state

reach the final size and defeat enemies or lose health/time. Score, accuracy, correct answers, and attempts map to the established `GameResults`; authoritative XP and persistence remain host-owned.
