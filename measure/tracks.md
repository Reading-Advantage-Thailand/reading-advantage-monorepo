...
- [~] **Track: Shared `packages/ai` + `lib/ai/` Refactor** *Link: [./tracks/ai_adapter_package_20260603/](./tracks/ai_adapter_package_20260603/)*
  *Status: IN PROGRESS — Phases 0–5 complete (`packages/ai` with `AIClient` interface, OpenAI/Google/Mock providers, `createAIClient`/`getAIClient` singleton). Phase 6 (recommendation-service refactor) complete. **Phase 7 (image-generator refactor) Red-phase complete 2026-06-06 (commit `2fd5887`)**; Green implementation pending. Phases 8–10 (SDK removal, docs, closeout) pending. Note: track directory was previously (and incorrectly) listed under `./archive/` while still in flight — the plan source of truth is `measure/tracks/ai_adapter_package_20260603/plan.md`.*
