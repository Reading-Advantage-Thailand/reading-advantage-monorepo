...
- [~] **Track: APK Action and Defense Evidence Cohort** *Link: [./tracks/apk_corpus_audit_action_defense_20260712/](./tracks/apk_corpus_audit_action_defense_20260712/)*
  Recover exact independently reviewed evidence packages for eight action, defense, escort, projectile, and arena games. **Batch A: ACCEPTED-WITH-DISCLOSURE (conditional)** at `62ff2a4d`; its accepted manifest is `b096d911…2a54c` and DISC-001 remains a process-only carry-forward disclosure. **Batch B: STOP-LOSS ACTIVE** at `dd59e5e6`: Village Guardian, Archer's Revenge, and Storm the Castle Tower have source packages, 12/12 asset reconciliation, and B0-B3 truth gates passing (38 tests plus 3 subtests). B4/B5 are blocked: two browser runners failed (`agent-browser` exit 127; CDP/browser-harness attachment failure), leaving all three runnable dispositions audit-blocked; 87 role-provenance fields are explicitly unavailable; and B-TEST-002 leaves v2 browser/review/receipt selection outside the v3 machine gate. The fresh adversarial review re-derived 29 claims and 12/12 fixtures without envelope mismatch, but reports **1 Critical, 1 High, 1 Medium**. No Batch B candidate, product-owner acceptance, or accepted manifest is authorized. Per the two-cycle rule, product-owner direction is required before more remediation. Batch C and cohort reconciliation have not started. See `_orchestrator/LAST-BATCH-STATUS.md`.*
