...
- (2026-05-23, prisma_drizzle_reading_controllers) Prisma `where: { schoolId }` on relation-less models (e.g., `UserActivity` had no `schoolId`) failed silently at runtime. When migrating to Drizzle, route those filters through a join on `users.schoolId` — this fixes the bug while preserving the controller signature. Plan called `gameRankings`/`lessonRecords` Track 4 slices but they are PORT-AS-IS in the unified schema — verify table existence in `packages/db/src/schema/` before deferring.
- (2026-05-23, prisma_drizzle_reading_controllers) Prisma migrations directory often holds **two kinds** of SQL: (a) schema changes already replaced by the unified Drizzle migration and (b) custom raw SQL (CREATE MATERIALIZED VIEW, REFRESH triggers, indexes on matviews) that has NO equivalent in the Drizzle schema generator. Before deleting `prisma/migrations/`, `grep -l "MATERIALIZED VIEW\\|REFRESH" prisma/migrations/*/migration.sql` and move that subset to a preserved location (`db-migrations/legacy-matviews/`) with a README — otherwise prod-required objects vanish from the repo even though they still exist in the live DB.
