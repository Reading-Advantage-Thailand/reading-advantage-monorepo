...
| 2026-05-14 | codecamp_curriculum | Old placeholder curriculum modules remain in existing DBs | Medium | Resolved | Seed script is now fully idempotent with 18-module data and orphan cleanup. Fresh DBs get correct data. Reseed to fix existing DBs. |
...
| 2026-05-16 | codecamp_exercise_repos_20260515 | Phase 3+4 plan tasks (create actual GitHub repos M11–15, 17, 18 + portfolio repos) not completed | Medium | Open | Backend domain, API, webhooks, seed data, and tests are complete. Phase 4 portfolio data wired into getUserDashboard. Remaining: create 6 exercise repos + 3 portfolio repos on GitHub, push scaffolding + README + solution branches. Requires org credentials and manual/CI action outside the monorepo. |
| 2026-05-16 | codecamp_exercise_repos_20260515 | Webhook async LLM review pipeline has no retry or dead-letter queue on failure | Medium | Open | Fire-and-forget `runReview()` catches errors and marks as "reviewed" with error note, but transient failures (network, API rate limit) are not retried. Requires job queue (Redis/BullMQ) for proper retry with idempotency. |
| 2026-05-16 | codecamp_exercise_repos_20260515 | `getInstallationTokenForRepo` uses single `GITHUB_INSTALLATION_ID` — cannot support multi-org or per-repo installations | Low | Open | Hardcoded installation ID works for single-org setup. Fix: accept `owner` param → call `GET /app/installations?account_id={owner}` to resolve installation ID dynamically. Deferred: not needed until multi-org support is required. |
| 2026-05-16 | codecamp_exercise_repos_20260515 | `PORTFOLIO_PROJECTS` seed export is dead code — never imported by seed script or domain | Low | Open | Defined in `packages/db/src/seed/codecamp-curriculum-data.ts` but unused at runtime. Dashboard uses hardcoded `PHASE_METADATA` instead. Either wire `PORTFOLIO_PROJECTS` into seed (add portfolio_projects table) or replace `PHASE_METADATA` in domain with import. Deferred: requires new DB table + schema or layer violation fix. |
