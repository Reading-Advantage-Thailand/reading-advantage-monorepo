...
| 2026-05-14 | codecamp_advantage | Mock-DB tests don't catch real DB constraint violations | Medium | Open | Deferred: requires integration test infrastructure against real Postgres. Seed data phase complete; clean test data makes integration tests feasible in a future track. |
| 2026-05-14 | codecamp_curriculum | Old placeholder curriculum modules remain in existing DBs | Medium | Open | Deferred: 18-module seed replaces 5-module data in fresh DBs. Needs cleanup migration for existing DBs — operational task, not codebase. |
...
| 2026-05-15 | codecamp_advantage | Hardcoded `moduleSlug === "real-world-practice"` for Module 18 workflow tracker | Medium | Open | Deferred: requires GitHub Issues API integration and practice repo setup (external). Hardcoded slug works until live API is connected. |
| 2026-05-15 | codecamp_advantage | Multiple PR reviews render duplicate WorkflowTrackers | Medium | Open | Deferred: needs UX design decision on how to present multi-exercise progress. Current behavior is functional but confusing. |
...
| 2026-05-15 | codecamp_review | `contentJson` schema validation missing in LessonContent | Low | Open | Deferred: component has defensive runtime guards (Array.isArray + filter). Adding Zod schemas for 3 lesson types is feasible but low urgency since data is seed-controlled. |
...
| 2026-05-15 | codecamp_review | `getExerciseRepos` generates repos for modules without exercises | Low | Open | Deferred: M1 and M18 have no exercise lessons. Design decision needed on whether to return empty array or placeholder repos. Low urgency. |
...
| 2026-05-15 | codecamp_review | Duplicate `generateReview` LLM implementation | Medium | Open | Deferred: extracting shared helper to `@reading-advantage/domain` would couple domain to the OpenRouter SDK. Both callers already inject `generateReview` into `reviewExercise`, so deduplication can be done by extracting just the OpenRouter caller, but requires coordinated package changes. |
| 2026-05-15 | codecamp_review | No integration tests for webhook → LLM → comment → DB pipeline | Medium | Open | Deferred: fire-and-forget async pattern means end-to-end happy path untested against real DB. Requires test database infrastructure and Redis for rate limiting. |
