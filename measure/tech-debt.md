...
| 2026-05-02 | unified_auth | science-advantage auth still uses Prisma (not Drizzle) | Medium | Open | Science has working username/password auth via Prisma. Needs Prisma-to-Drizzle migration to use shared auth package. Deferred — no real users yet. |
...
| 2026-05-02 | unified_auth | Drizzle migration not generated for unified auth schema | Medium | Open | Schema rewritten in Phase 1 but `drizzle-kit generate` was deferred. Must run before deploying to production. |
...
| 2026-05-02 | shared_config | @reading-advantage/ui has few shared components | Low | Open | Only ~5 components exported. Top 10 shadcn components from reading-advantage should be migrated. Deferred from shared_config_consolidation. |
...
| 2026-05-03 | firestore_drizzle | reading-advantage controllers still use Prisma not Drizzle | High | Open | user-controller, license-controller, generator-controller all use `prisma.*` directly. Needs Prisma→Drizzle migration to complete unification. |
