...
| 2026-05-14 | codecamp_advantage | Mock-DB tests don't catch real DB constraint violations | Medium | Open | Requires real-DB integration test infrastructure. Phase 5 seed data (85 lessons, 18 modules) provides clean foundation for when infra is built. |
...
| 2026-05-15 | codecamp_advantage | Module 18 WorkflowTracker uses hardcoded issue data (title, number, steps) | Medium | Open | Requires GitHub Issues API client + external repo with filed issues. Not addressable by seed data phase. |
| 2026-05-15 | codecamp_advantage | Multiple PR reviews render duplicate WorkflowTrackers | Medium | Open | Needs UX design decision on grouping multi-exercise progress. Phase 7 renders one tracker per PR review; grouping by exercise repo requires data reshaping and UX approval. |
