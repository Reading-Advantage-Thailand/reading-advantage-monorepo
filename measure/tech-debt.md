...
| 2026-05-16 | codecamp_exercise_repos_20260515 | `getInstallationTokenForRepo` uses single `GITHUB_INSTALLATION_ID` — cannot support multi-org or per-repo installations | Low | Open | Hardcoded installation ID works for single-org setup. Fix: accept `owner` param → call `GET /app/installations?account_id={owner}` to resolve installation ID dynamically. Phase 5 docs (`github-app-setup.md`) document single-org assumption. Deferred: not needed until multi-org support is required. |
| 2026-05-16 | codecamp_exercise_repos_20260515 | `PORTFOLIO_PROJECTS` seed export is dead code — never imported by seed script or domain | Low | Resolved | Phase 5 review: exported `PORTFOLIO_PROJECTS` from `@reading-advantage/db` and wired domain's `PHASE_METADATA` to derive portfolio data from it, eliminating duplication. |
