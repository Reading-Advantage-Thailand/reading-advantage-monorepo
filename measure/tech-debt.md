...
| 2026-05-14 | codecamp_advantage | Mock-DB tests don't catch real DB constraint violations | Medium | Open | Deferred to future track: requires real-DB integration test infrastructure. Phase 7 complete; seed data provides clean test data foundation for when infra is built. |
...
| 2026-05-15 | codecamp_advantage | Module 18 WorkflowTracker uses hardcoded issue data (title, number, steps) | Medium | Open | Deferred: requires GitHub Issues API client + external repo with filed issues. Phase 7 renders WorkflowTracker dynamically from review status; real issue data requires infra beyond this codebase. |
| 2026-05-15 | codecamp_advantage | Multiple PR reviews render duplicate WorkflowTrackers | Medium | Open | Deferred: needs UX design decision on grouping multi-exercise progress. Phase 7 renders one tracker per PR review; grouping by exercise repo requires data reshaping and UX approval. |
...
| 2026-05-15 | codecamp_review | `contentJson` schema validation missing in LessonContent | Low | Open | Deferred: component has defensive runtime guards. Adding Zod schemas for 3 lesson types is feasible but low urgency — data is seed-controlled and phase complete. |
...
| 2026-05-15 | codecamp_review | Duplicate `generateReview` LLM implementation | Medium | Open | Deferred: extracting shared helper to `@reading-advantage/domain` would couple domain to OpenRouter SDK. Both callers already inject `generateReview` into `reviewExercise`. Deduplication needs coordinated package changes; track complete. |
| 2026-05-15 | codecamp_review | No integration tests for webhook → LLM → comment → DB pipeline | Medium | Open | Deferred: fire-and-forget async pattern means E2E happy path untested. Requires test database infrastructure and Redis for rate limiting. Track complete; deferred to future infra track. |
