...
| 2026-05-16 | codecamp_exercise_repos_20260515 | GitHub webhook handler fetches all exercise repos to match by URL; should query by repoUrl directly | Low | Open | Requires new domain function `getExerciseRepoByUrl`. Minor optimization — only 16 repos, negligible impact at current scale. |
| 2026-05-16 | codecamp_exercise_repos_20260515 | `linkExerciseRepo` domain function lacks module existence validation before insert | Low | Open | Relies on DB FK constraint for error handling. Adding explicit check would provide clearer error messages. |