...
| 2026-05-16 | codecamp_exercise_repos_20260515 | GitHub webhook handler fetches all exercise repos to match by URL; should query by repoUrl directly | Low | **Resolved** | Fixed: added `getExerciseRepoByUrl` domain function; webhook now queries by repoUrl directly instead of fetching all repos. |
| 2026-05-16 | codecamp_exercise_repos_20260515 | `linkExerciseRepo` domain function lacks module existence validation before insert | Low | **Resolved** | Fixed: `linkExerciseRepo` now queries `codecampModules` before insert and throws `"Module not found"` if the module doesn't exist. |