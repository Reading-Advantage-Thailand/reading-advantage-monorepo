...
class ExistingCoreTask4AcceptanceTests(unittest.TestCase):
    ...
    def test_acceptance_pins_exact_task4_implementation_bytes(self) -> None:
        """Requires exhaustive bindings to match their immutable acceptance revision."""
        acceptance = _load_object(ACCEPTANCE_PATH)
        ...
        self.assertFalse(successor["authorization"]["final_cohort_acceptance_authorized"])
    ...
    def test_plan_metadata_and_index_preserve_task4_and_bound_later_progression(self) -> None:
        ...
        task6_line = next(line for line in plan.splitlines() if "Delete only each title's exact replaced legacy paths" in line)
