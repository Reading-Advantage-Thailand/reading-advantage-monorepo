...
class R1V3ExecutionClosureRedTests(unittest.TestCase):
    ...
    def test_preseal_materialize_failure_evidence_collision_preserves_existing_attempt_and_retains_cause(self) -> None:
        """Requires a competing public attempt to survive a causally bound collision failure.

        @returns Nothing; real preservation and validation use temporary roots, one competing mkdir, and no runner execution.
        """
        self.assertFalse(V3_DIR.exists())
        podman = importlib.import_module("measure.business_operations_graph_baseline_execution_closure_v3_podman")
        executor_type = getattr(podman, "DirectCommandRuntimeProductionExecutorV1", None)
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            stderr = staged_raw / "receipt-materialize.stderr.txt"
            stdout.write_bytes(b"")
            stderr.write_bytes(b"V3_TEST_PRESEAL_COLLISION_FAILURE\\n")
            logical_argv = ["node", "materialize-v3"]
            payload_argv = [
                podman.CONTAINER_NODE,
                "/runner/materialize.mjs",
            ...
            sentinel_name = "pre-existing-collision-sentinel.txt"
            sentinel_bytes = b"V3_TEST_PRESEAL_COLLISION_SENTINEL\\n"
            ...
            def validate_private_attempt(attempt: dict[str, Any], directory: Path | str) -> None:
                """Validates and records one complete private canonical pre-seal record.
            ...
            with patch.object(
                ...
                podman.os,
                "rename",
                side_effect=AssertionError("V3_PRESEAL_COLLISION_RENAME_CALLED"),
            ), patch.object(
                podman.os,
                "replace",
                side_effect=AssertionError("V3_PRESEAL_COLLISION_REPLACE_CALLED"),
            ), patch.object(
                podman,
                "_publish_candidate_publication_failure_attempt",
                side_effect=AssertionError("V3_PRESEAL_COLLISION_CANDIDATE_PUBLISHER_CALLED"),
            ), patch.object(
                podman,
                "_publish_blocker",
                side_effect=AssertionError("V3_PRESEAL_COLLISION_GENERIC_BLOCKER_CALLED"),
            ), patch.object(
                podman,
                "_run_container",
                side_effect=AssertionError("V3_PRESEAL_COLLISION_PODMAN_CALLED"),
            ), patch.object(
                podman,
                "capture_direct_command_runtime_in_container_trace_v1",
                side_effect=AssertionError("V3_PRESEAL_COLLISION_TRACE_CALLED"),
            ), patch.object(
                executor,
                "runtime_build",
                side_effect=AssertionError("V3_PRESEAL_COLLISION_LATER_STAGE_CALLED"),
            ), patch.object(
                executor,
                "generate",
                side_effect=AssertionError("V3_PRESEAL_COLLISION_LATER_STAGE_CALLED"),
                ...
                with self.assertRaisesRegex(
                    podman.CandidateExecutionBlocked,
                    rf"^V3_PODMAN_FAILURE_EVIDENCE_UNPRESERVED: materialize: V3_PODMAN_ATTEMPT_PUBLICATION_COLLISION: {expected_attempt_name}$",
                ) as raised:
                    executor.preserve_failure(materialize_error)
    ...
    def test_preseal_materialize_failure_evidence_json_write_failure_cleans_private_stage_and_retains_cause(self) -> None:
        """Requires a pre-seal failed-attempt JSON write failure to retain its materialize cause.

        @returns Nothing; real preservation and finalization use temporary roots and one deterministic JSON-write failure only.
        """
        self.assertFalse(V3_DIR.exists())
        podman = importlib.import_module("measure.business_operations_graph_baseline_execution_closure_v3_podman")
        executor_type = getattr(podman, "DirectCommandRuntimeProductionExecutorV1", None)
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            stdout = staged_raw / "receipt-materialize.stdout.txt"
            stderr = staged_raw / "receipt-materialize.stderr.txt"
            stdout_bytes = b"V3_TEST_PRESEAL_JSON_WRITE_STDOUT_3B7D02\\n"
            stderr_bytes = b"V3_TEST_PRESEAL_JSON_WRITE_STDERR_9E14C8\\n"
            stdout.write_bytes(stdout_bytes)
            stderr.write_bytes(stderr_bytes)
            logical_argv = ["node", "materialize-v3"]
            payload_argv = [
    ...
    def test_preseal_materialize_failure_evidence_reservation_cleanup_failure_does_not_mask_original_failure(self) -> None:
        """Requires a reservation cleanup failure never to mask the rename and materialize failures.

        @returns Nothing; real preservation uses temporary roots, one injected rename failure, and one injected reservation cleanup failure only.
        """
        self.assertFalse(V3_DIR.exists())
        podman = importlib.import_module("measure.business_operations_graph_baseline_execution_closure_v3_podman")
        executor_type = getattr(podman, "DirectCommandRuntimeProductionExecutorV1", None)
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            stdout = staged_raw / "receipt-materialize.stdout.txt"
            stderr = staged_raw / "receipt-materialize.stderr.txt"
            stdout_bytes = b"V3_TEST_PRESEAL_RESERVATION_CLEANUP_STDOUT_2E71C4\\n"
            stderr_bytes = b"V3_TEST_PRESEAL_RESERVATION_CLEANUP_STDERR_9A3B52\\n"
            stdout.write_bytes(stdout_bytes)
            stderr.write_bytes(stderr_bytes)
            logical_argv = ["node", "materialize-v3"]
            payload_argv = [
            ...
            cleanup_error = PermissionError("V3_TEST_PRESEAL_FAILURE_EVIDENCE_CLEANUP")
            expected_attempt_name = f"{podman.ATTEMPT_PREFIX}-20260802-0001"
            final_reservation = attempts_root / expected_attempt_name
            validated_directories: list[Path] = []
            validated_public_paths_absent: list[bool] = []
            rename_calls: list[tuple[Path, Path]] = []
            rmtree_calls: list[Path] = []
            ...
            def fail_rename(source: Path | str, destination: Path | str) -> None:
                ...
                rename_calls.append((private_directory, final_directory))
                self.assertEqual(private_directory, validated_directories[0])
                self.assertEqual(final_directory, final_reservation)
                self.assertTrue(final_directory.is_dir())
                self.assertFalse(final_directory.is_symlink())
                self.assertEqual(list(final_directory.iterdir()), [])
                raise rename_error

            def fail_reservation_cleanup(path: str | Path, *args: Any, **kwargs: Any) -> None:
                """Removes the reservation then reports the reservation cleanup as a deterministic fault.

                @param path The cleanup target requested by the real publisher.
                @param args Any positional options forwarded by the real caller.
                @param kwargs Any keyword options forwarded by the real caller.
                @returns Nothing because the recorded reservation cleanup always raises after removal.
                """
                cleanup_path = Path(path)
                rmtree_calls.append(cleanup_path)
                native_rmtree(cleanup_path, *args, **kwargs)
                if cleanup_path == final_reservation:
                    raise cleanup_error
            ...
            with patch.object(
                ...
                podman.os,
                "replace",
                side_effect=AssertionError("V3_PRESEAL_RESERVATION_CLEANUP_REPLACE_CALLED"),
            ), patch.object(
                podman,
                "_publish_candidate_publication_failure_attempt",
                side_effect=AssertionError("V3_PRESEAL_RESERVATION_CLEANUP_CANDIDATE_PUBLISHER_CALLED"),
            ), patch.object(
                podman,
                "_publish_blocker",
                side_effect=AssertionError("V3_PRESEAL_RESERVATION_CLEANUP_GENERIC_BLOCKER_CALLED"),
            ), patch.object(
                podman,
                "_run_container",
                side_effect=AssertionError("V3_PRESEAL_RESERVATION_CLEANUP_PODMAN_CALLED"),
            ), patch.object(
                podman,
                "capture_direct_command_runtime_in_container_trace_v1",
                side_effect=AssertionError("V3_PRESEAL_RESERVATION_CLEANUP_TRACE_CALLED"),
            ), patch.object(
                executor,
                "runtime_build",
                side_effect=AssertionError("V3_PRESEAL_RESERVATION_CLEANUP_LATER_STAGE_CALLED"),
            ), patch.object(
                executor,
                "generate",
                side_effect=AssertionError("V3_PRESEAL_RESERVATION_CLEANUP_LATER_STAGE_CALLED"),
                ...
                podman.shutil,
                "rmtree",
                side_effect=fail_reservation_cleanup,
            ), patch.object(podman, "TRACK_DIR", attempts_root):
                with self.assertRaisesRegex(
                    podman.CandidateExecutionBlocked,
                    r"^V3_PODMAN_FAILURE_EVIDENCE_UNPRESERVED: materialize: V3_TEST_PRESEAL_FAILURE_EVIDENCE_RENAME$",
            ...
            self.assertEqual(
                rename_calls,
                [(validated_directories[0], final_reservation)],
            )
            self.assertEqual(len(rmtree_calls), 2)
            self.assertEqual(rmtree_calls[0], final_reservation)
            self.assertEqual(rmtree_calls[1], validated_directories[0].parent)
            self.assertTrue(rmtree_calls[1].name.startswith(".failed-attempt-"))
            self.assertFalse(final_reservation.exists())
            self.assertFalse(rmtree_calls[1].exists())
            self.assertFalse(output.exists())
            self.assertFalse(V3_DIR.exists())
            self.assertFalse(validated_directories[0].exists())
            self.assertFalse((attempts_root / "raw").exists())
    ...
    def test_candidate_failure_evidence_reservation_cleanup_failure_does_not_mask_original_failure(self) -> None:
        """Requires a reservation cleanup failure never to mask the candidate rename failure.

        @returns Nothing; real preservation stages privately, validates with the real validator, and injects one rename failure plus one reservation cleanup failure only.
        """
        self.assertFalse(V3_DIR.exists())
        podman = importlib.import_module("measure.business_operations_graph_baseline_execution_closure_v3_podman")
        executor_type = getattr(podman, "DirectCommandRuntimeProductionExecutorV1", None)
        ...
        package_root = "packages/fixture-candidate-evidence-reservation-cleanup"
        generator_bytes, source_bytes = b"export {};\n", b"candidate-reservation-cleanup\n"
        generator = identity(f"{package_root}/scripts/generate-runtime.mjs", generator_bytes)
        source = identity(f"{package_root}/assets/standard/input.txt", source_bytes)
        baseline_reads = sorted([generator, source], key=lambda item: item["path"])
        output_path = f"{package_root}/assets/standard/standard-pack-release.json"
        ...
        read_set = {
            "schemaVersion": 1, "kind": "direct-command-runtime-read-set",
            "trigger": {
                "logicalArgv": ["pnpm", "--filter", "@fixture/candidate-reservation-cleanup", "fixture-build"],
                "package": "@fixture/candidate-reservation-cleanup",
                "manifest": {"path": f"{package_root}/package.json", "sha256": "d" * 64, "size": 1},
            },
            "baselineReadSet": baseline_reads, "derivedBuildReadSet": [derived], "outputPaths": [output_path],
            "preflightQuota": {
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            rename_error = OSError("V3_TEST_CANDIDATE_FAILURE_EVIDENCE_RENAME")
            cleanup_error = PermissionError("V3_TEST_CANDIDATE_FAILURE_EVIDENCE_CLEANUP")
            expected_attempt_name = f"{podman.ATTEMPT_PREFIX}-20260802-0001"
            final_reservation = attempts_root / expected_attempt_name
            validated_directories: list[Path] = []
            validated_public_paths_absent: list[bool] = []
            ...
            def fail_rename(source: Path | str, destination: Path | str) -> None:
                ...
                rename_calls.append((private_directory, final_path))
                self.assertEqual(private_directory, validated_directories[0])
                self.assertEqual(final_path, final_reservation)
                self.assertTrue(final_path.is_dir())
                self.assertFalse(final_path.is_symlink())
                self.assertEqual(list(final_path.iterdir()), [])
                raise rename_error

            def fail_reservation_cleanup(path: str | Path, *args: Any, **kwargs: Any) -> None:
                """Removes the reservation then reports only that cleanup as a deterministic fault.
            ...
            with patch.object(
                ...
                podman,
                "_publish_blocker",
                side_effect=AssertionError("V3_CANDIDATE_FAILURE_EVIDENCE_GENERIC_BLOCKER_CALLED"),
            ), patch.object(
                podman,
                "_run_container",
                side_effect=AssertionError("V3_CANDIDATE_FAILURE_EVIDENCE_PODMAN_CALLED"),
            ), patch.object(
                podman,
                "capture_direct_command_runtime_in_container_trace_v1",
                side_effect=AssertionError("V3_CANDIDATE_FAILURE_EVIDENCE_TRACE_CALLED"),
            ), patch.object(
                executor,
                "runtime_build",
                side_effect=AssertionError("V3_CANDIDATE_FAILURE_EVIDENCE_LATER_STAGE_CALLED"),
            ), patch.object(
                executor,
                "generate",
                side_effect=AssertionError("V3_CANDIDATE_FAILURE_EVIDENCE_LATER_STAGE_CALLED"),
                ...
                with self.assertRaisesRegex(
                    podman.CandidateExecutionBlocked,
                    r"^V3_PODMAN_FAILURE_EVIDENCE_UNPRESERVED: candidate-publication: V3_TEST_CANDIDATE_FAILURE_EVIDENCE_RENAME$",
                ) as raised:
                    executor.preserve_failure(candidate_error)
            ...
            self.assertEqual(
                rename_calls,
                [(private_directory, final_reservation)],
            )
            self.assertEqual(len(rmtree_calls), 1)
            self.assertEqual(rmtree_calls[0], final_reservation)
            self.assertFalse(final_reservation.exists())
            self.assertFalse(private_directory.exists())
            self.assertFalse(private_directory.parent.exists())
            self.assertFalse(output.exists())
            self.assertFalse(V3_DIR.exists())
            self.assertEqual(sorted(path.name for path in attempts_root.iterdir()), [])

    def test_candidate_failure_evidence_rename_failure_cleans_reservation_and_retains_cause(self) -> None:
        """Requires a candidate-publisher rename failure to retain its operation cause.

        @returns Nothing; real preservation stages privately, validates with the real validator, and injects one rename failure only.
        """
        self.assertFalse(V3_DIR.exists())
        podman = importlib.import_module("measure.business_operations_graph_baseline_execution_closure_v3_podman")
        executor_type = getattr(podman, "DirectCommandRuntimeProductionExecutorV1", None)
        ...
        build_failure = getattr(podman, "_build_candidate_publication_failure_v1", None)
        real_validator = getattr(podman, "validate_failed_execution_attempt_v1", None)
        real_write_json = getattr(podman, "_write_json", None)
        error_type = getattr(importlib.import_module(HELPER_MODULE), "ExecutionClosureValidationError", None)
        self.assertTrue(inspect.isclass(executor_type), "V3_DIRECT_RUNTIME_PRODUCTION_EXECUTOR_MISSING")
        self.assertTrue(callable(build_integration), "V3_DIRECT_RUNTIME_INTEGRATION_BUILDER_MISSING")
        self.assertTrue(callable(validate_integration), "V3_DIRECT_RUNTIME_INTEGRATION_VALIDATOR_MISSING")
        self.assertTrue(callable(build_failure), "V3_CANDIDATE_FAILURE_BUILDER_MISSING")
        self.assertTrue(callable(real_validator), "V3_PODMAN_FAILED_ATTEMPT_VALIDATOR_MISSING")
        self.assertTrue(callable(real_write_json), "V3_CANDIDATE_FAILURE_EVIDENCE_JSON_WRITER_MISSING")
        self.assertTrue(isinstance(error_type, type) and issubclass(error_type, Exception))
        ...
        package_root = "packages/fixture-candidate-evidence-rename-causality"
        generator_bytes, source_bytes = b"export {};\n", b"candidate-rename-causality\n"
        generator = identity(f"{package_root}/scripts/generate-runtime.mjs", generator_bytes)
        source = identity(f"{package_root}/assets/standard/input.txt", source_bytes)
        baseline_reads = sorted([generator, source], key=lambda item: item["path"])
        output_path = f"{package_root}/assets/standard/standard-pack-release.json"
        ...
        read_set = {
            "schemaVersion": 1, "kind": "direct-command-runtime-read-set",
            "trigger": {
                "logicalArgv": ["pnpm", "--filter", "@fixture/candidate-rename-causality", "fixture-build"],
                "package": "@fixture/candidate-rename-causality",
                "manifest": {"path": f"{package_root}/package.json", "sha256": "d" * 64, "size": 1},
            },
            "baselineReadSet": baseline_reads, "derivedBuildReadSet": [derived], "outputPaths": [output_path],
            "preflightQuota": {
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            def record_json_write(path, value) -> None:
                """Records the sole private failed-attempt write and forwards it to the real writer.
                ...
                @returns Nothing after the real writer persists the private record.
                """
                json_write_calls.append(Path(path))
            ...
            with patch.object(
                ...
                podman.os,
                "replace",
                side_effect=AssertionError("V3_CANDIDATE_RENAME_CAUSALITY_REPLACE_CALLED"),
            ), patch.object(
                podman,
                "_publish_blocker",
                side_effect=AssertionError("V3_CANDIDATE_RENAME_CAUSALITY_GENERIC_BLOCKER_CALLED"),
            ), patch.object(
                podman,
                "_run_container",
                side_effect=AssertionError("V3_CANDIDATE_RENAME_CAUSALITY_PODMAN_CALLED"),
            ), patch.object(
                podman,
                "capture_direct_command_runtime_in_container_trace_v1",
                side_effect=AssertionError("V3_CANDIDATE_RENAME_CAUSALITY_TRACE_CALLED"),
            ), patch.object(
                executor,
                "runtime_build",
                side_effect=AssertionError("V3_CANDIDATE_RENAME_CAUSALITY_LATER_STAGE_CALLED"),
            ), patch.object(
                executor,
                "generate",
                side_effect=AssertionError("V3_CANDIDATE_RENAME_CAUSALITY_LATER_STAGE_CALLED"),
            ...
            self.assertIs(raised.exception.__cause__, rename_error)
            self.assertIs(rename_error.__cause__, candidate_error)
            self.assertEqual(len(validated_directories), 1)
            self.assertEqual(validated_public_paths_absent, [True])
            private_directory = validated_directories[0]
            self.assertEqual(
            ...
            self.assertEqual(json_write_calls, [private_directory / "failed-attempt.json"])
            self.assertFalse(final_reservation.exists())
            self.assertFalse(private_directory.exists())
            self.assertFalse(private_directory.parent.exists())
            self.assertFalse(output.exists())
